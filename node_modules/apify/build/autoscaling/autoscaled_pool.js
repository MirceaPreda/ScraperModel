...
var _snapshotter = _interopRequireDefault(require("./snapshotter"));

var _system_status = _interopRequireDefault(require("./system_status"));
...
class AutoscaledPool {
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(maxConcurrency, 'options.maxConcurrency', 'Number');
    (0, _utils.checkParamOrThrow)(minConcurrency, 'options.minConcurrency', 'Number');
    (0, _utils.checkParamOrThrow)(desiredConcurrency, 'options.desiredConcurrency', 'Maybe Number');
    (0, _utils.checkParamOrThrow)(desiredConcurrencyRatio, 'options.desiredConcurrencyRatio', 'Number');
    (0, _utils.checkParamOrThrow)(scaleUpStepRatio, 'options.scaleUpStepRatio', 'Number');
    (0, _utils.checkParamOrThrow)(scaleDownStepRatio, 'options.scaleDownStepRatio', 'Number');
    (0, _utils.checkParamOrThrow)(maybeRunIntervalSecs, 'options.maybeRunIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(loggingIntervalSecs, 'options.loggingIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(autoscaleIntervalSecs, 'options.autoscaleIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(runTaskFunction, 'options.runTaskFunction', 'Function');
    (0, _utils.checkParamOrThrow)(isFinishedFunction, 'options.isFinishedFunction', 'Function');
    (0, _utils.checkParamOrThrow)(isTaskReadyFunction, 'options.isTaskReadyFunction', 'Function');
    (0, _utils.checkParamOrThrow)(systemStatusOptions, 'options.systemStatusOptions', 'Maybe Object');
    (0, _utils.checkParamOrThrow)(snapshotterOptions, 'options.snapshotterOptions', 'Maybe Object'); // Configurable properties.
    ...
    this.isTaskReadyFunction = isTaskReadyFunction; // Internal properties.
    ...
    this._maybeRunTask = this._maybeRunTask.bind(this); // Create instances with correct options.

    const ssoCopy = Object.assign({}, systemStatusOptions);
    if (!ssoCopy.snapshotter) ssoCopy.snapshotter = new _snapshotter.default(snapshotterOptions);
    this.snapshotter = ssoCopy.snapshotter;
    this.systemStatus = new _system_status.default(ssoCopy);
  }
  /**
   * @ignore
   */
  ...
  setMaxConcurrency(maxConcurrency) {
    _log.default.deprecated('AutoscaledPool.setMaxConcurrency() is deprecated, use the "maxConcurrency" property instead');
  ...
  set minConcurrency(value) {
    (0, _utils.checkParamOrThrow)(value, 'value', 'Number');
  ...
  async run() {
    ...
    await this.snapshotter.start(); // This interval checks the system status and updates the desired concurrency accordingly.

    this.autoscaleInterval = (0, _utilities.betterSetInterval)(this._autoscale, this.autoscaleIntervalMillis); // This is here because if we scale down to let's say 1, then after each promise is finished
    ...
    this.maybeRunInterval = (0, _utilities.betterSetInterval)(this._maybeRunTask, this.maybeRunIntervalMillis);
  ...
  async pause(timeoutSecs) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (timeoutSecs) {
        timeout = setTimeout(() => {
          const err = new Error('AutoscaledPool: The pool\'s running tasks did not finish' + `in ${timeoutSecs} secs after pool.pause() invocation.`);
          reject(err);
        }, timeoutSecs);
      }
  ...
  async _maybeRunTask(intervalCallback) {
    ...
    const done = intervalCallback || (() => {}); // Prevent starting a new task if:
    // - the pool is paused or aborted


    if (this.isStopped) return done(); // - we are already querying for a task.

    if (this.queryingIsTaskReady) return done(); // - we would exceed desired concurrency.

    if (this._currentConcurrency >= this._desiredConcurrency) return done(); // - system is overloaded now and we are at or above minConcurrency
    ...
    if (!isSystemIdle && this._currentConcurrency >= this._minConcurrency) {
      _log.default.perf('AutoscaledPool: Task will not be run. System is overloaded.', currentStatus);
    ...
    try {
      ...
      if (this.reject) {
        ...
        _log.default.exception(err, 'AutoscaledPool: isTaskReadyFunction failed');
    ...
    if (!isTaskReady) {
      done(); // No tasks could mean that we're finished with all tasks.
    ...
    try {
      ...
      this._currentConcurrency++; // Try to run next task to build up concurrency,
      // but defer it so it doesn't create a cycle.

      setImmediate(this._maybeRunTask); // We need to restart interval here, so that it doesn't get blocked by a stalled task.

      done(); // Execute the current task.

      await this.runTaskFunction();
      this._currentConcurrency--; // Run task after the previous one finished.
      ...
      if (this.reject) {
        // No need to log all concurrent errors.
        _log.default.exception(err, 'AutoscaledPool: runTaskFunction failed.');
  ...
  _autoscale(intervalCallback) {
    ...
    if (this.isStopped) return intervalCallback(); // Only scale up if:
    // - system has not been overloaded lately.

    const systemStatus = this.systemStatus.getHistoricalStatus();
    const {
      isSystemIdle
    } = systemStatus; // - we're not already at max concurrency.

    const weAreNotAtMax = this._desiredConcurrency < this._maxConcurrency; // - current concurrency reaches at least the given ratio of desired concurrency.
    ...
    if (isSystemIdle && weAreNotAtMax && weAreReachingDesiredConcurrency) this._scaleUp(systemStatus); // Always scale down if:
    // - the system has been overloaded lately.

    const isSystemOverloaded = !isSystemIdle; // - we're over min concurrency.

    const weAreNotAtMin = this._desiredConcurrency > this._minConcurrency;
    if (isSystemOverloaded && weAreNotAtMin) this._scaleDown(systemStatus); // On periodic intervals, print comprehensive log information
    ...
    if (this.loggingIntervalMillis > 0) {
      ...
      if (now > this.lastLoggingTime + this.loggingIntervalMillis) {
        ...
        _log.default.info('AutoscaledPool state', {
  ...
  _scaleUp(systemStatus) {
    ...
    _log.default.debug('AutoscaledPool: scaling up', {
  ...
  _scaleDown(systemStatus) {
    ...
    _log.default.debug('AutoscaledPool: scaling down', {
      oldConcurrency: this._desiredConcurrency + step,
      newConcurrency: this._desiredConcurrency,
      systemStatus
    });
  ...
  async _maybeFinish() {
    ...
    try {
      ...
      if (this.reject) {
        // No need to log all concurrent errors.
        _log.default.exception(err, 'AutoscaledPool: isFinishedFunction failed.');
  ...
  async _destroy() {
    this.resolve = null;
    this.reject = null;
    (0, _utilities.betterClearInterval)(this.autoscaleInterval);
    (0, _utilities.betterClearInterval)(this.maybeRunInterval);
    if (this.snapshotter) await this.snapshotter.stop();
  }

}

var _default = AutoscaledPool;
