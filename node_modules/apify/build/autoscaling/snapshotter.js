...
class Snapshotter {
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(eventLoopSnapshotIntervalSecs, 'options.eventLoopSnapshotIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(memorySnapshotIntervalSecs, 'options.memorySnapshotIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(cpuSnapshotIntervalSecs, 'options.cpuSnapshotIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(snapshotHistorySecs, 'options.snapshotHistorySecs', 'Number');
    (0, _utils.checkParamOrThrow)(clientSnapshotIntervalSecs, 'options.clientSnapshotIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(maxBlockedMillis, 'options.maxBlockedMillis', 'Number');
    (0, _utils.checkParamOrThrow)(maxUsedMemoryRatio, 'options.maxUsedMemoryRatio', 'Number');
    (0, _utils.checkParamOrThrow)(maxUsedCpuRatio, 'options.maxUsedCpuRatio', 'Number');
    (0, _utils.checkParamOrThrow)(maxClientErrors, 'options.maxClientErrors', 'Number');
    ...
    this.maxMemoryBytes = parseInt(process.env[_consts.ENV_VARS.MEMORY_MBYTES], 10) * 1024 * 1024 || null;
    this.cpuSnapshots = [];
    ...
    this.lastLoggedCriticalMemoryOverloadAt = null; // We need to pre-bind those functions to be able to successfully remove listeners.
  ...
  async start() {
    await this._ensureCorrectMaxMemory(); // Start snapshotting.

    this.eventLoopInterval = (0, _utilities.betterSetInterval)(this._snapshotEventLoop.bind(this), this.eventLoopSnapshotIntervalMillis);
    this.clientInterval = (0, _utilities.betterSetInterval)(this._snapshotClient.bind(this), this.clientSnapshotIntervalMillis);

    if ((0, _utils2.isAtHome)()) {
      _events.default.on(_consts.ACTOR_EVENT_NAMES.SYSTEM_INFO, this._snapshotCpuOnPlatform);

      _events.default.on(_consts.ACTOR_EVENT_NAMES.SYSTEM_INFO, this._snapshotMemoryOnPlatform);
    } else {
      this.cpuInterval = (0, _utilities.betterSetInterval)(this._snapshotCpuOnLocal.bind(this), this.cpuSnapshotIntervalMillis);
      this.memoryInterval = (0, _utilities.betterSetInterval)(this._snapshotMemoryOnLocal.bind(this), this.memorySnapshotIntervalMillis);
    }
  }
  /**
  ...
  async stop() {
    (0, _utilities.betterClearInterval)(this.eventLoopInterval);
    (0, _utilities.betterClearInterval)(this.memoryInterval);
    (0, _utilities.betterClearInterval)(this.cpuInterval);
    (0, _utilities.betterClearInterval)(this.clientInterval);

    _events.default.removeListener(_consts.ACTOR_EVENT_NAMES.SYSTEM_INFO, this._snapshotCpuOnPlatform);

    _events.default.removeListener(_consts.ACTOR_EVENT_NAMES.SYSTEM_INFO, this._snapshotMemoryOnPlatform); // Allow microtask queue to unwind before stop returns.
  ...
  _getSample(snapshots, sampleDurationMillis) {
    ...
    while (idx--) {
      ...
      if (latestTime - snapshot.createdAt <= sampleDurationMillis) sample.unshift(snapshot);else break;
    }
  ...
  async _snapshotMemoryOnLocal(intervalCallback) {
    try {
      const now = new Date();
      const memInfo = await (0, _utils2.getMemoryInfo)();
      const {
        mainProcessBytes,
        childProcessesBytes
      } = memInfo;
      ...
      _log.default.exception(err, 'Snapshotter: Memory snapshot failed.');
    } finally {
      intervalCallback();
    }
  }
  ...
  _memoryOverloadWarning({
    memCurrentBytes
  }) {
    const now = new Date();
    if (this.lastLoggedCriticalMemoryOverloadAt && now.getTime() < this.lastLoggedCriticalMemoryOverloadAt.getTime() + CRITICAL_OVERLOAD_RATE_LIMIT_MILLIS) return;
    ...
    if (isCriticalOverload) {
      ...
      _log.default.warning('Memory is critically overloaded. ' + `Using ${toMb(memCurrentBytes)} MB of ${toMb(this.maxMemoryBytes)} MB (${usedPercentage}%). Consider increasing the actor memory.`);
  ...
  _snapshotClient(intervalCallback) {
    ...
    const allErrorCounts = _utils2.apifyClient.stats.rateLimitErrors;
    const currentErrCount = allErrorCounts[CLIENT_RATE_LIMIT_ERROR_RETRY_COUNT] || 0; // Handle empty snapshots array
  ...
  _pruneSnapshots(snapshots, now) {
    ...
    for (let i = 0; i < snapshots.length; i++) {
      ...
      if (now.getTime() - createdAt > this.snapshotHistoryMillis) oldCount++;else break;
    }
  ...
  async _ensureCorrectMaxMemory() {
    ...
    const {
      totalBytes
    } = await (0, _utils2.getMemoryInfo)();

    if ((0, _utils2.isAtHome)()) {
      this.maxMemoryBytes = totalBytes;
    } else {
      this.maxMemoryBytes = Math.ceil(totalBytes / 4); // NOTE: Log as AutoscaledPool, so that users are not confused what "Snapshotter" is

      _log.default.info(`AutoscaledPool: Setting max memory of this run to ${Math.round(this.maxMemoryBytes / 1024 / 1024)} MB. Use the ${_consts.ENV_VARS.MEMORY_MBYTES} environment variable to override it.`); // eslint-disable-line max-len
