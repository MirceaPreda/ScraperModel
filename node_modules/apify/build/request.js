...
exports.computeUniqueKey = computeUniqueKey;
...
function computeUniqueKey({
  ...
  const normalizedUrl = (0, _utilities.normalizeUrl)(url, keepUrlFragment) || url; // It returns null when url is invalid, causing weird errors.
  ...
  if (!useExtendedUniqueKey) {
    if (normalizedMethod !== 'GET' && payload) {
      ...
      _log.default.deprecated(`We've encountered a ${normalizedMethod} Request with a payload. ` + 'This is fine. Just letting you know that if your requests point to the same URL ' + 'and differ only in method and payload, you should see the "useExtendedUniqueKey" option of Request constructor.');
    }
...
function hashPayload(payload) {
  return _crypto.default.createHash('sha256').update(payload).digest('base64').replace(/(\+|\/|=)/g, '').substr(0, 8);
}
/**
...
class Request {
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(id, 'id', 'Maybe String');
    (0, _utils.checkParamOrThrow)(url, 'url', 'String');
    (0, _utils.checkParamOrThrow)(loadedUrl, 'url', 'Maybe String');
    (0, _utils.checkParamOrThrow)(uniqueKey, 'uniqueKey', 'Maybe String');
    (0, _utils.checkParamOrThrow)(method, 'method', 'String');
    (0, _utils.checkParamOrThrow)(payload, 'payload', 'Maybe Buffer | String');
    (0, _utils.checkParamOrThrow)(noRetry, 'noRetry', 'Boolean');
    (0, _utils.checkParamOrThrow)(retryCount, 'retryCount', 'Number');
    (0, _utils.checkParamOrThrow)(errorMessages, 'errorMessages', 'Maybe Array');
    (0, _utils.checkParamOrThrow)(headers, 'headers', 'Object');
    (0, _utils.checkParamOrThrow)(userData, 'userData', 'Object');
    (0, _utils.checkParamOrThrow)(handledAt, 'handledAt', 'Maybe String | Date');
    (0, _utils.checkParamOrThrow)(keepUrlFragment, 'keepUrlFragment', 'Boolean');
    (0, _utils.checkParamOrThrow)(useExtendedUniqueKey, 'useExtendedUniqueKey', 'Boolean');
    ...
    this.uniqueKey = uniqueKey || computeUniqueKey({
      url,
      method,
      payload,
      keepUrlFragment,
    ...
    this.userData = JSON.parse(JSON.stringify(userData)); // Requests received from API will have ISOString dates,
    // but we want to have a Date instance.
    // eslint-disable-next-line no-nested-ternary

    this.handledAt = _underscore.default.isDate(handledAt) ? handledAt : handledAt ? new Date(handledAt) : null;
  }
  /**
  ...
  pushErrorMessage(errorOrMessage, options = {}) {
    ...
    if (type === 'object') {
      if (!errorOrMessage) {
        ...
        message = omitStack ? errorOrMessage.message // .stack includes the message
        ...
        try {
          message = _util.default.inspect(errorOrMessage);
        } catch (err) {
  ...
  doNotRetry(message) {
    _log.default.deprecated('request.doNotRetry is deprecated. Use request.noRetry = true; instead.');
