...
var _streams_utilities = require("apify-shared/streams_utilities");
...
var utilsRequest = _interopRequireWildcard(require("../utils_request"));
...
class CheerioCrawler {
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(prepareRequestFunction, 'options.prepareRequestFunction', 'Maybe Function');
    (0, _utils.checkParamOrThrow)(additionalMimeTypes, 'options.additionalMimeTypes', '[String]');
    ...
    (0, _utils.checkParamOrThrow)(persistCookiesPerSession, 'options.persistCookiesPerSession', 'Boolean'); // Enforce valid proxy configuration
  ...
  async _handleRequestFunction({
    ...
    if (this.prepareRequestFunction) await this.prepareRequestFunction({
      request,
      session
    });
    const {
      ...
      contentType,
      response
    } = await (0, _utils2.addTimeoutToPromise)(this._requestFunction({
      request,
      session
    }), this.requestTimeoutMillis, `CheerioCrawler: request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);
    ...
    const $ = dom ? _cheerio.default.load(dom, {
      xmlMode: isXml
    }) : null;
    const context = {
      $,
      ...
      get html() {
        _log.default.deprecated('CheerioCrawler: The "html" parameter of handlePageFunction is deprecated, use "body" instead.');

        return dom && !isXml && $.html({
    ...
    return (0, _utils2.addTimeoutToPromise)(this.handlePageFunction(context), this.handlePageTimeoutMillis, `CheerioCrawler: handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
  }
  /**
  ...
  async _requestFunction({
    ...
    const {
      type,
      charset
    } = (0, _utils2.parseContentTypeFromResponse)(responseStream);

    const {
      response,
      encoding
    } = this._encodeResponse(request, responseStream, charset);
    ...
    if (statusCode >= 500) {
      const body = await (0, _streams_utilities.readStreamToString)(response, encoding); // Errors are often sent as JSON, so attempt to parse them,
      ...
      return {
        dom,
        isXml: type.includes('xml'),
        response,
        contentType
      };
    } else {
      const body = await (0, _streams_utilities.concatStreamToBuffer)(response);
      return {
        body,
        response,
        contentType
  ...
  _getRequestOptions(request, session) {
    const mandatoryRequestOptions = {
      ...
      abortFunction: res => {
        ...
        const {
          type
        } = (0, _utils2.parseContentTypeFromResponse)(res);
        ...
        if (!this.supportedMimeTypes.has(type) && statusCode < 500) {
          request.noRetry = true;
          throw new Error(`CheerioCrawler: Resource ${request.url} served Content-Type ${type}, ` + `but only ${Array.from(this.supportedMimeTypes).join(', ')} are allowed. Skipping resource.`);
        }
  ...
  _encodeResponse(request, response, encoding) {
    if (!encoding || Buffer.isEncoding(encoding)) return {
      response,
      encoding
    };
  ...
  async _parseHtmlToDom(response) {
    return new Promise((resolve, reject) => {
      const domHandler = new _htmlparser.default.DomHandler((err, dom) => {
        if (err) reject(err);else resolve(dom);
      });
      const parser = new _htmlparser.default.Parser(domHandler, {
        decodeEntities: true
      });
  ...
  _extendSupportedMimeTypes(additionalMimeTypes) {
    additionalMimeTypes.forEach(mimeType => {
      try {
        const parsedType = _contentType.default.parse(mimeType);
