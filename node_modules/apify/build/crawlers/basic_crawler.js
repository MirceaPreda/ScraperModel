...
const DEFAULT_OPTIONS = {
  ...
  handleFailedRequestFunction: ({
    ...
    _log.default.error('BasicCrawler: Request failed and reached maximum retries', details);
  },
  autoscaledPoolOptions: {},
  sessionPoolOptions: {},
  // We could add sessionPool true/false config to use/not use SessionPool.
...
class BasicCrawler {
  constructor(options) {
    ...
    (0, _utilities.checkParamPrototypeOrThrow)(requestList, 'options.requestList', _request_list.RequestList, 'Apify.RequestList', true);
    (0, _utilities.checkParamPrototypeOrThrow)(requestQueue, 'options.requestQueue', [_request_queue.RequestQueue, _request_queue.RequestQueueLocal], 'Apify.RequestQueue', true);
    (0, _utils.checkParamOrThrow)(handleRequestFunction, 'options.handleRequestFunction', 'Function');
    (0, _utils.checkParamOrThrow)(handleRequestTimeoutSecs, 'options.handleRequestTimeoutSecs', 'Number');
    (0, _utils.checkParamOrThrow)(handleFailedRequestFunction, 'options.handleFailedRequestFunction', 'Function');
    (0, _utils.checkParamOrThrow)(maxRequestRetries, 'options.maxRequestRetries', 'Number');
    (0, _utils.checkParamOrThrow)(maxRequestsPerCrawl, 'options.maxRequestsPerCrawl', 'Maybe Number');
    (0, _utils.checkParamOrThrow)(autoscaledPoolOptions, 'options.autoscaledPoolOptions', 'Object');
    ...
    this.stats = new _statistics.default({
      logMessage: 'Crawler request statistics:'
    });
    ...
    const basicCrawlerAutoscaledPoolConfiguration = {
      ...
      isTaskReadyFunction: async () => {
        if (isMaxPagesExceeded()) {
          if (shouldLogMaxPagesExceeded) {
            _log.default.info('BasicCrawler: Crawler reached the maxRequestsPerCrawl limit of ' + `${maxRequestsPerCrawl} requests and will shut down soon. Requests that are in progress will be allowed to finish.`);
      ...
      isFinishedFunction: async () => {
        if (isMaxPagesExceeded()) {
          _log.default.info(`BasicCrawler: Earlier, the crawler reached the maxRequestsPerCrawl limit of ${maxRequestsPerCrawl} requests ` + 'and all requests that were in progress at that time have now finished. ' + `In total, the crawler processed ${this.handledRequestsCount} requests and will shut down.`);
        ...
        if (isFinished) {
          const reason = isFinishedFunction ? 'BasicCrawler: Crawler\'s custom isFinishedFunction() returned true, the crawler will shut down.' : 'BasicCrawler: All the requests from request list and/or request queue have been processed, the crawler will shut down.';

          _log.default.info(reason);
        }

        return isFinished;
      }
    };
    this.autoscaledPoolOptions = _underscore.default.defaults({}, basicCrawlerAutoscaledPoolConfiguration, autoscaledPoolOptions);
    this.isRunningPromise = null; // Attach a listener to handle migration events gracefully.

    _events.default.on(_consts.ACTOR_EVENT_NAMES.MIGRATING, this._pauseOnMigration.bind(this));
  }
  /**
  ...
  async run() {
    if (this.isRunningPromise) return this.isRunningPromise; // Initialize AutoscaledPool before awaiting _loadHandledRequestCount(),
    // so that the caller can get a reference to it before awaiting the promise returned from run()
    // (otherwise there would be no way)

    this.autoscaledPool = new _autoscaled_pool.default(this.autoscaledPoolOptions);
    ...
    try {
      ...
      _log.default.info('Crawler final request statistics:', finalStats);
    }
  }
  ...
  async _pauseOnMigration() {
    await this.autoscaledPool.pause(SAFE_MIGRATION_WAIT_MILLIS).catch(err => {
      if (err.message.includes('running tasks did not finish')) {
        _log.default.error('BasicCrawler: The crawler was paused due to migration to another host, ' + 'but some requests did not finish in time. Those requests\' results may be duplicated.');
      } else {
        throw err;
      }
    });
    ...
    if (this.requestList) {
      ...
      await this.requestList.persistState().catch(err => {
        if (err.message.includes('Cannot persist state.')) {
          _log.default.error('BasicCrawler: The crawler attempted to persist its request list\'s state and failed due to missing or ' + 'invalid config. Make sure to use either Apify.openRequestList() or the "stateKeyPrefix" option of RequestList ' + 'constructor to ensure your crawling state is persisted through host migrations and restarts.');
        } else {
          _log.default.exception(err, 'BasicCrawler: An unexpected error occured when the crawler ' + 'attempted to persist its request list\'s state.');
        }
      });
    }
  }
  ...
  async _fetchNextRequest() {
    ...
    try {
      ...
      _log.default.exception(err, 'RequestQueue.addRequest() failed, reclaiming request back to the list', {
        request
      });
    ...
    const [nextRequest] = await Promise.all([this.requestQueue.fetchNextRequest(), this.requestList.markRequestHandled(request)]);
    return nextRequest;
  }
  /**
  ...
  async _runTaskFunction() {
    ...
    if (!request) return; // Reset loadedUrl so an old one is not carried over to retries.
    ...
    try {
      await (0, _utils2.addTimeoutToPromise)(this.handleRequestFunction({
        request,
        autoscaledPool: this.autoscaledPool,
        session
      }), this.handleRequestTimeoutMillis, `BasicCrawler: handleRequestFunction timed out after ${this.handleRequestTimeoutMillis / 1000} seconds.`);
      await source.markRequestHandled(request);
      this.stats.finishJob(statisticsId);
      this.handledRequestsCount++; // reclaim session if request finishes successfully
      ...
      try {
        ...
        _log.default.exception(secondaryError, 'BasicCrawler: runTaskFunction error handler threw an exception. ' + 'This places the crawler and its underlying storages into an unknown state and crawling will be terminated. ' + 'This may have happened due to an internal error of Apify\'s API or due to a misconfigured crawler. ' + 'If you are sure that there is no error in your code, selecting "Restart on error" in the actor\'s settings' + 'will make sure that the run continues where it left off, if programmed to handle restarts correctly.');
  ...
  async _isTaskReadyFunction() {
    ...
    const isRequestListEmpty = this.requestList ? await this.requestList.isEmpty() : true; // If RequestList is not empty, task is ready, no reason to check RequestQueue.

    if (!isRequestListEmpty) return true; // If RequestQueue is not empty, task is ready, return true, otherwise false.
  ...
  async _defaultIsFinishedFunction() {
    const [isRequestListFinished, isRequestQueueFinished] = await Promise.all([this.requestList ? this.requestList.isFinished() : true, this.requestQueue ? this.requestQueue.isFinished() : true]); // If both are finished, return true, otherwise return false.
  ...
  async _requestFunctionErrorHandler(error, request, source) {
    request.pushErrorMessage(error); // Reclaim and retry request if flagged as retriable and retryCount is not exceeded.

    if (!request.noRetry && request.retryCount < this.maxRequestRetries) {
      request.retryCount++;

      _log.default.exception(error, 'BasicCrawler: handleRequestFunction failed, reclaiming failed request back to the list or queue', _underscore.default.pick(request, 'url', 'retryCount', 'id'));
    ...
    return this.handleFailedRequestFunction({
      request,
      error
