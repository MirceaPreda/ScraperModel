...
async function enqueueLinksByClickingElements(options = {}) {
  ...
  (0, _utils.checkParamOrThrow)(selector, 'selector', 'String');
  ...
  (0, _utils.checkParamOrThrow)(pseudoUrls, 'pseudoUrls', 'Maybe Array');
  (0, _utils.checkParamOrThrow)(transformRequestFunction, 'transformRequestFunction', 'Function');
  (0, _utils.checkParamOrThrow)(waitForPageIdleSecs, 'waitForPageIdleSecs', 'Number');
  (0, _utils.checkParamOrThrow)(maxWaitForPageIdleSecs, 'maxWaitForPageIdleSecs', 'Number');
  const waitForPageIdleMillis = waitForPageIdleSecs * 1000;
  const maxWaitForPageIdleMillis = maxWaitForPageIdleSecs * 1000;
  const pseudoUrlInstances = (0, _shared.constructPseudoUrlInstances)(pseudoUrls || []);
  ...
  let requestOptions = (0, _shared.createRequestOptions)(interceptedRequests);

  if (transformRequestFunction) {
    requestOptions = requestOptions.map(transformRequestFunction).filter(r => !!r);
  }

  const requests = (0, _shared.createRequests)(requestOptions, pseudoUrlInstances);
  return (0, _shared.addRequestsToQueueInBatches)(requests, requestQueue);
}
/**
...
async function clickElementsAndInterceptNavigationRequests(options) {
  ...
  await (0, _puppeteer_request_interception.addInterceptRequestHandler)(page, onInterceptedRequest);
  ...
  await (0, _puppeteer_request_interception.removeInterceptRequestHandler)(page, onInterceptedRequest);
...
function createTargetCreatedHandler(page, requests) {
  return async function onTargetCreated(target) {
    ...
    try {
      ...
      _log.default.debug('enqueueLinksByClickingElements: Could not close spawned page.', {
        error: err.stack
      });
    }
  };
...
async function clickElements(page, selector) {
  ...
  _log.default.debug(`enqueueLinksByClickingElements: There are ${elementHandles.length} elements to click.`);
  ...
  for (const handle of elementHandles) {
    try {
      ...
      if (shouldLogWarning && err.stack.includes('is detached from document')) {
        _log.default.warning(`An element with selector ${selector} that you're trying to click has been removed from the page. ` + 'This was probably caused by an earlier click which triggered some JavaScript on the page that caused it to change. ' + 'If you\'re trying to enqueue pagination links, we suggest using the "next" button, if available and going one by one.');

        shouldLogWarning = false;
      }

      _log.default.debug('enqueueLinksByClickingElements: Click failed.', {
        stack: err.stack
      });
    }
  }

  _log.default.debug(`enqueueLinksByClickingElements: Successfully clicked ${clickedElementsCount} elements out of ${elementHandles.length}`);
}
/* istanbul ignore next */
...
async function waitForPageIdle({
  ...
  return new Promise(resolve => {
    ...
    function maxTimeoutHandler() {
      _log.default.debug(`enqueueLinksByClickingElements: Page still showed activity after ${maxWaitForPageIdleMillis}ms. ` + 'This is probably due to the website itself dispatching requests, but some links may also have been missed.');
...
async function restoreHistoryNavigationAndSaveCapturedUrls(page, requests) {
  ...
  stateHistory.forEach(args => {
    try {
      ...
      const url = new _url.URL(stateUrl, page.url()).href;
      requests.add(JSON.stringify({
        url
      }));
    } catch (err) {
      _log.default.debug('enqueueLinksByClickingElements: Failed to ', {
        error: err.stack
      });
    }
  });
