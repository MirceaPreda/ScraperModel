...
exports.addWebhook = exports.getApifyProxyUrl = exports.metamorph = exports.callTask = exports.call = exports.main = exports.getEnv = void 0;
...
var _regexs = require("apify-shared/regexs");
...
const waitForRunToFinish = async ({
  actId,
  runId,
  token,
  waitSecs,
  ...
  const shouldRepeat = () => {
    ...
    if (updatedRun && _consts.ACT_JOB_TERMINAL_STATUSES.includes(updatedRun.status)) return false;
    return true;
  };
  ...
  while (shouldRepeat()) {
    getRunOpts.waitForFinish = waitSecs ? Math.round(waitSecs - (Date.now() - startedAt) / 1000) : 999999;
    updatedRun = await acts.getRun(getRunOpts); // It might take some time for database replicas to get up-to-date,
    // so getRun() might return null. Wait a little bit and try it again.

    if (!updatedRun) await (0, _utils2.sleep)(250);
  }

  if (!updatedRun) {
    throw new _errors.ApifyCallError({
      id: runId,
      actId
  ...
  if (status !== _consts.ACT_JOB_STATUSES.SUCCEEDED && status !== _consts.ACT_JOB_STATUSES.RUNNING && status !== _consts.ACT_JOB_STATUSES.READY) {
    const message = taskId ? `The actor task ${taskId} invoked by Apify.call() did not succeed. For details, see https://my.apify.com/view/runs/${runId}` : `The actor ${actId} invoked by Apify.call() did not succeed. For details, see https://my.apify.com/view/runs/${runId}`;
    throw new _errors.ApifyCallError(updatedRun, message);
  }
...
const addInputOptionsOrThrow = (input, contentType, options) => {
  options.contentType = contentType; // NOTE: this function modifies contentType property on options object if needed.

  options.body = (0, _key_value_store.maybeStringify)(input, options);
  (0, _utils.checkParamOrThrow)(options.body, 'input', 'Buffer|String');
  (0, _utils.checkParamOrThrow)(options.contentType, 'contentType', 'String');
  options.contentType = (0, _utils2.addCharsetToContentType)(options.contentType);
};
/**
...
const getEnv = () => {
  ...
  _underscore.default.mapObject(_consts.ENV_VARS, (fullName, shortName) => {
    const camelCaseName = (0, _utils2.snakeCaseToCamelCase)(shortName);
    let value = env[fullName]; // Parse dates and integers.

    if (value && fullName.endsWith('_AT')) value = tryParseDate(value);else if (_underscore.default.contains(_consts.INTEGER_ENV_VARS, fullName)) value = parseInt(value, 10);
...
const main = userFunc => {
  ...
  (0, _utils2.logSystemInfo)(); // Log warning if SDK is outdated.

  (0, _utils2.printOutdatedSdkWarning)();

  if (!process.env[_consts.ENV_VARS.LOCAL_STORAGE_DIR] && !process.env[_consts.ENV_VARS.TOKEN]) {
    const dir = _path.default.join(process.cwd(), './apify_storage');

    process.env[_consts.ENV_VARS.LOCAL_STORAGE_DIR] = dir;

    _log.default.warning(`Neither ${_consts.ENV_VARS.LOCAL_STORAGE_DIR} nor ${_consts.ENV_VARS.TOKEN} environment variable is set, defaulting to ${_consts.ENV_VARS.LOCAL_STORAGE_DIR}="${dir}"`); // eslint-disable-line max-len
  ...
  const exitWithError = (err, exitCode, message) => {
    _log.default.exception(err, message);

    exited = true; // console.log(`Exiting with code: ${exitCode}`);
  ...
  const intervalId = setInterval(_underscore.default.noop, 9999999); // Using async here to have nice stack traces for errors

  const run = async () => {
    (0, _events.initializeEvents)();

    try {
      await userFunc();
      (0, _events.stopEvents)();
      ...
      if (!exited) {
        exitWithError(err, _constants.EXIT_CODES.ERROR_USER_FUNCTION_THREW, 'The function passed to Apify.main() threw an exception:');
      }
    }
  };
...
const call = async (actId, input, options = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(options, 'opts', 'Object'); // Common options.

  const {
    token
  } = options;
  (0, _utils.checkParamOrThrow)(token, 'token', 'Maybe String'); // RunAct() options.
  ...
  if (memory && !memoryMbytes) {
    ...
    if (!callMemoryWarningIssued) {
      callMemoryWarningIssued = true; // eslint-disable-next-line max-len

      _log.default.warning('The "memory" option of the Apify.call() function has been deprecated and will be removed in the future. Use "memoryMbytes" instead!');
    }
  }

  (0, _utils.checkParamOrThrow)(build, 'build', 'Maybe String');
  (0, _utils.checkParamOrThrow)(memoryMbytes, 'memoryMbytes', 'Maybe Number');
  (0, _utils.checkParamOrThrow)(timeoutSecs, 'timeoutSecs', 'Maybe Number');
  (0, _utils.checkParamOrThrow)(webhooks, 'webhooks', 'Maybe Array');
  ...
  if (input) addInputOptionsOrThrow(input, options.contentType, runActOpts); // Run actor.

  const {
    waitSecs
  } = options;
  (0, _utils.checkParamOrThrow)(waitSecs, 'waitSecs', 'Maybe Number');
  ...
  if (!fetchOutput || updatedRun.status !== _consts.ACT_JOB_STATUSES.SUCCEEDED) return updatedRun; // Fetch output.

  const {
    disableBodyParser = false
  } = options;
  (0, _utils.checkParamOrThrow)(disableBodyParser, 'disableBodyParser', 'Boolean');
  const output = await keyValueStores.getRecord({
    key: 'OUTPUT',
    storeId: updatedRun.defaultKeyValueStoreId,
    disableBodyParser
  });
  return Object.assign({}, updatedRun, {
    output
  });
};
/**
...
const callTask = async (taskId, input, options = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(taskId, 'taskId', 'String');
  (0, _utils.checkParamOrThrow)(options, 'opts', 'Object'); // Common options.

  const {
    token
  } = options;
  (0, _utils.checkParamOrThrow)(token, 'token', 'Maybe String'); // Run task options.
  ...
  if (webhooks) runTaskOpts.webhooks = webhooks; // Start task.
...
const metamorph = async (targetActorId, input, options = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(targetActorId, 'targetActorId', 'String');
  (0, _utils.checkParamOrThrow)(optionsCopy, 'opts', 'Object');
  (0, _utils.checkParamOrThrow)(optionsCopy.build, 'options.build', 'Maybe String');
  (0, _utils.checkParamOrThrow)(optionsCopy.contentType, 'options.contentType', 'Maybe String');
  const actorId = process.env[_consts.ENV_VARS.ACTOR_ID];
  const runId = process.env[_consts.ENV_VARS.ACTOR_RUN_ID];
  if (!actorId) throw new Error(`Environment variable ${_consts.ENV_VARS.ACTOR_ID} must be provided!`);
  if (!runId) throw new Error(`Environment variable ${_consts.ENV_VARS.ACTOR_RUN_ID} must be provided!`);

  if (input) {
    input = (0, _key_value_store.maybeStringify)(input, optionsCopy);
    (0, _utils.checkParamOrThrow)(input, 'input', 'Buffer|String');
    if (optionsCopy.contentType) optionsCopy.contentType = (0, _utils2.addCharsetToContentType)(optionsCopy.contentType);
  }
  ...
  await (0, _utils2.sleep)(optionsCopy.customAfterSleepMillis || METAMORPH_AFTER_SLEEP_MILLIS);
};
/**
...
const getApifyProxyUrl = (options = {}) => {
  // For backwards compatibility.
  // TODO: remove this when we release v1.0.0
  if (!options.groups && options.apifyProxyGroups) {
    _log.default.warning('Parameter `apifyProxyGroups` of Apify.getApifyProxyUrl() is deprecated!!! Use `groups` instead!');

    options.groups = options.apifyProxyGroups;
  }

  if (!options.session && options.apifyProxySession) {
    _log.default.warning('Parameter `apifyProxySession` of Apify.getApifyProxyUrl() is deprecated!!! Use `session` instead!');
  ...
  const {
    ...
    session,
    country,
    password = process.env[_consts.ENV_VARS.PROXY_PASSWORD],
    hostname = process.env[_consts.ENV_VARS.PROXY_HOSTNAME] || _consts.LOCAL_ENV_VARS[_consts.ENV_VARS.PROXY_HOSTNAME],
    port = parseInt(process.env[_consts.ENV_VARS.PROXY_PORT] || _consts.LOCAL_ENV_VARS[_consts.ENV_VARS.PROXY_PORT], 10),
  ...
  (0, _utils.checkParamOrThrow)(groups, groupsParamName, 'Maybe [String]');
  (0, _utils.checkParamOrThrow)(session, sessionParamName, 'Maybe Number | String');
  (0, _utils.checkParamOrThrow)(country, countryParamName, 'Maybe String');
  (0, _utils.checkParamOrThrow)(password, 'opts.password', 'String', getMissingParamErrorMgs('password', _consts.ENV_VARS.PROXY_PASSWORD));
  (0, _utils.checkParamOrThrow)(hostname, 'opts.hostname', 'String', getMissingParamErrorMgs('hostname', _consts.ENV_VARS.PROXY_HOSTNAME));
  (0, _utils.checkParamOrThrow)(port, 'opts.port', 'Number', getMissingParamErrorMgs('port', _consts.ENV_VARS.PROXY_PORT));
  let username;
  ...
  if (groups || session || country) {
    ...
    if (groups && groups.length) {
      if (!groups.every(group => _regexs.APIFY_PROXY_VALUE_REGEX.test(group))) throwInvalidProxyValueError('groups');
      parts.push(`groups-${groups.join('+')}`);
    }

    if (session) {
      if (!_regexs.APIFY_PROXY_VALUE_REGEX.test(session)) throwInvalidProxyValueError('session');
      parts.push(`session-${session}`);
    }

    if (country) {
      if (!_constants.COUNTRY_CODE_REGEX.test(country)) throwInvalidCountryCode(country);
...
exports.getApifyProxyUrl = getApifyProxyUrl;
...
const addWebhook = async ({
  ...
  (0, _utils.checkParamOrThrow)(eventTypes, 'eventTypes', '[String]');
  (0, _utils.checkParamOrThrow)(requestUrl, 'requestUrl', 'String');
  (0, _utils.checkParamOrThrow)(payloadTemplate, 'payloadTemplate', 'Maybe String');

  if (!(0, _utils2.isAtHome)()) {
    _log.default.warning('Apify.addWebhook() is only supported when running on the Apify platform. The webhook will not be invoked.');
  ...
  if (!runId) {
    throw new Error(`Environment variable ${_consts.ENV_VARS.ACTOR_RUN_ID} is not set!`);
  }

  return _utils2.apifyClient.webhooks.createWebhook({
    webhook: {
      isAdHoc: true,
      eventTypes,
      condition: {
