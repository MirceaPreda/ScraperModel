...
exports.openRequestList = exports.RequestList = exports.SOURCES_PERSISTENCE_KEY = exports.STATE_PERSISTENCE_KEY = void 0;
...
exports.STATE_PERSISTENCE_KEY = STATE_PERSISTENCE_KEY;
...
exports.SOURCES_PERSISTENCE_KEY = SOURCES_PERSISTENCE_KEY;
...
class RequestList {
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(sources, 'options.sources', 'Array');
    (0, _utils.checkParamOrThrow)(state, 'options.state', 'Maybe Object');
    (0, _utils.checkParamOrThrow)(persistStateKey, 'options.persistStateKey', 'Maybe String');
    (0, _utils.checkParamOrThrow)(persistSourcesKey, 'options.persistSourcesKey', 'Maybe String');
    (0, _utils.checkParamOrThrow)(keepDuplicateUrls, 'options.keepDuplicateUrls', 'Maybe Boolean'); // Array of all requests from all sources, in the order as they appeared in sources.
    // All requests in the array have distinct uniqueKey!

    this.requests = []; // Index to the next item in requests array to fetch. All previous requests are either handled or in progress.

    this.nextIndex = 0; // Dictionary, key is Request.uniqueKey, value is corresponding index in the requests array.

    this.uniqueKeyToIndex = {}; // Dictionary of requests that were returned by fetchNextRequest().
    // The key is uniqueKey, value is true.
    // TODO: Change this to Set

    this.inProgress = {}; // Dictionary of requests for which reclaimRequest() was called.
    ...
    this.initialState = state; // If this option is set then all requests will get a pre-generated unique ID and duplicate URLs will be kept in the list.

    this.keepDuplicateUrls = keepDuplicateUrls; // Starts as true because until we handle the first request, the list is effectively persisted by doing nothing.

    this.isStatePersisted = true; // Starts as false because we don't know yet and sources might change in the meantime (eg. download from live list).
  ...
  async initialize() {
    ...
    this.isLoading = true;
    const [state, sources] = await this._loadStateAndSources(); // If there are no sources, it just means that we've not persisted any (yet).

    if (sources) this.areSourcesPersisted = true;
    const actualSources = sources || this.sources; // We'll load all sources in sequence to ensure that they get loaded in the right order.
    ...
    if (this.persistStateKey) {
      _events.default.on(_constants.ACTOR_EVENT_NAMES_EX.PERSIST_STATE, this.persistState.bind(this));
    }
  }
  /**
  ...
  async persistState() {
    ...
    try {
      await (0, _key_value_store.setValue)(this.persistStateKey, this.getState());
      this.isStatePersisted = true;
    } catch (err) {
      _log.default.exception(err, 'RequestList attempted to persist state, but failed.');
    }
  }
  /**
  ...
  async _persistSources() {
    await (0, _key_value_store.setValue)(this.persistSourcesKey, this.sources);
  ...
  _restoreState(state) {
    ...
    if (!state) return; // Restore previous state.
    ...
    _underscore.default.keys(state.inProgress).forEach(uniqueKey => {
    ...
    if (deleteFromInProgress.length) {
      _log.default.warning('RequestList\'s in-progress field is not consistent, skipping invalid in-progress entries', {
        deleteFromInProgress
      });

      _underscore.default.each(deleteFromInProgress, uniqueKey => {
        delete state.inProgress[uniqueKey];
      });
    }

    this.nextIndex = state.nextIndex;
    this.inProgress = state.inProgress; // All in-progress requests need to be recrawled

    this.reclaimed = _underscore.default.clone(this.inProgress);
  }
  /**
  ...
  async _loadStateAndSources() {
    ...
    if (this.initialState) {
      _log.default.debug('RequestList: Loading previous state from options.state argument.');

      state = this.initialState;
    } else if (this.persistStateKey) {
      _log.default.debug('RequestList: Loading previous state from key value store using the persistStateKey.');

      state = (0, _key_value_store.getValue)(this.persistStateKey);
    }

    if (this.persistSourcesKey) {
      _log.default.debug('RequestList: Loading sources from key value store using the persistSourcesKey.');

      return Promise.all([state, (0, _key_value_store.getValue)(this.persistSourcesKey)]);
    }
  ...
  async isEmpty() {
    ...
    return !(0, _utils2.getFirstKey)(this.reclaimed) && this.nextIndex >= this.requests.length;
  }
  /**
  ...
  async isFinished() {
    ...
    return !(0, _utils2.getFirstKey)(this.inProgress) && this.nextIndex >= this.requests.length;
  }
  /**
  ...
  async fetchNextRequest() {
    this._ensureIsInitialized(); // First return reclaimed requests if any.


    const uniqueKey = (0, _utils2.getFirstKey)(this.reclaimed);
  ...
  async _addFetchedRequests(source, fetchedRequests) {
    ...
    _log.default.info('RequestList: list fetched.', {
      requestsFromUrl,
      regex,
      fetchedCount,
      importedCount,
  ...
  async _fetchRequestsFromUrl(source) {
    const sharedOpts = _underscore.default.omit(source, 'requestsFromUrl', 'regex');
    ...
    const {
      downloadListOfUrls
    } = _utils2.publicUtils; // Download remote resource and parse URLs.
    ...
    if (!urlsArr.length) {
      _log.default.warning('RequestList: list fetched, but it is empty.', {
        requestsFromUrl,
        regex
      });
    ...
    return urlsArr.map(url => _underscore.default.extend({
      url
    }, sharedOpts));
  }
  /**
  ...
  _addRequest(opts) {
    ...
    const request = opts instanceof _request.default ? opts : new _request.default(opts); // Add index to uniqueKey if duplicates are to be kept
    ...
    this._ensureUniqueKeyValid(uniqueKey); // Skip requests with duplicate uniqueKey
    ...
    if (!this.uniqueKeyToIndex.hasOwnProperty(uniqueKey)) {
      ...
      _log.default.warning(`RequestList: Duplicate uniqueKey: ${uniqueKey} found while the keepDuplicateUrls option was set. Check your sources' unique keys.`); // eslint-disable-line max-len
  ...
  handledCount() {
    ...
    return this.nextIndex - _underscore.default.size(this.inProgress);
  }
...
const openRequestList = async (listName, sources, options = {}) => {
  (0, _utils.checkParamOrThrow)(listName, 'listName', 'String | Null');
  (0, _utils.checkParamOrThrow)(sources, 'sources', '[Object | String]');
  if (!sources.length) throw new Error('Parameter sources must not be an empty array.');
  (0, _utils.checkParamOrThrow)(options, 'options', 'Object'); // Support both an array of strings and array of objects.

  if (typeof sources[0] === 'string') sources = sources.map(url => ({
    url
  }));
