...
var _click_elements = require("./enqueue_links/click_elements");

var _puppeteer_request_interception = require("./puppeteer_request_interception");
...
const readFilePromised = _util.default.promisify(_fs.default.readFile);
...
const hideWebDriver = async page => {
  _log.default.deprecated('Apify.utils.puppeteer.hideWebDriver() is deprecated. Use launchPuppeteerOptions.stealth instead.');

  (0, _utils.checkParamOrThrow)(page, 'page', 'Object');
...
const injectedFilesCache = new _lru_cache.default({
...
const injectFile = async (page, filePath, options = {}) => {
  (0, _utils.checkParamOrThrow)(page, 'page', 'Object');
  (0, _utils.checkParamOrThrow)(filePath, 'filePath', 'String');
  ...
  return options.surviveNavigations ? Promise.all([page.evaluateOnNewDocument(contents), evalP]) : evalP;
};
/**
...
const enqueueRequestsFromClickableElements = async (page, selector, purls, requestQueue, requestOpts = {}) => {
  _log.default.warning('Function enqueueRequestsFromClickableElements is deprecated!!! Use `enqueueLinks` instead!');

  (0, _utils.checkParamOrThrow)(page, 'page', 'Object');
  (0, _utils.checkParamOrThrow)(purls, 'purls', 'Array');
  (0, _utilities.checkParamPrototypeOrThrow)(requestQueue, 'requestQueue', [_request_queue.RequestQueue, _request_queue.RequestQueueLocal], 'Apify.RequestQueue');
  (0, _utils.checkParamOrThrow)(requestOpts, 'requestOpts', 'Object');
  /* istanbul ignore next */

  const getHrefs = linkEls => linkEls.map(link => link.href).filter(href => !!href);

  const matchesPseudoUrl = url => _underscore.default.some(purls, purl => purl.matches(url));

  const urls = await page.$$eval(selector, getHrefs);
  const requests = urls.filter(matchesPseudoUrl).map(url => new _request.default(Object.assign({
    url
  }, requestOpts)));
...
const blockRequests = async (page, options = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(urlPatterns, 'options.urlPatterns', '[String]');
  (0, _utils.checkParamOrThrow)(extraUrlPatterns, 'options.extraUrlPatterns', '[String]');
...
const blockResources = async (page, resourceTypes = ['stylesheet', 'font', 'image', 'media']) => {
  _log.default.deprecated('Apify.utils.puppeteer.blockResources() has a high impact on performance in recent versions of Puppeteer. ' + 'Until this resolves, please use Apify.utils.puppeteer.blockRequests()');

  await (0, _puppeteer_request_interception.addInterceptRequestHandler)(page, async request => {
    const type = request.resourceType();
    if (resourceTypes.includes(type)) await request.abort();else await request.continue();
  });
};
/**
...
const cacheResponses = async (page, cache, responseUrlRules) => {
  (0, _utils.checkParamOrThrow)(page, 'page', 'Object');
  (0, _utils.checkParamOrThrow)(cache, 'cache', 'Object');
  (0, _utils.checkParamOrThrow)(responseUrlRules, 'responseUrlRules', 'Array');

  _log.default.deprecated('Apify.utils.puppeteer.cacheResponses() has a high impact on performance ' + 'in recent versions of Puppeteer so it\'s use is discouraged until this issue resolves.'); // Check that rules are either String or RegExp


  responseUrlRules.forEach((rule, index) => (0, _utils.checkParamOrThrow)(rule, `responseUrlRules[${index}]`, 'String | RegExp'));
  await (0, _puppeteer_request_interception.addInterceptRequestHandler)(page, async request => {
  ...
  page.on('response', async response => {
    const url = response.url(); // Response is already cached, do nothing
    ...
    } catch (e) {// ignore errors, usualy means that buffer is empty or broken connection
    }
  });
};
/**
...
const compileScript = (scriptString, context = Object.create(null)) => {
  ...
  try {
    func = _vm.default.runInNewContext(funcString, context); // "Secure" the context by removing prototypes, unless custom context is provided.
  } catch (err) {
    _log.default.exception(err, 'Cannot compile script!');

    throw err;
  }

  if (!_underscore.default.isFunction(func)) throw new Error('Compilation result is not a function!'); // This should not happen...
...
const gotoExtended = async (page, request, gotoOptions = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(gotoOptions, 'gotoOptions', 'Object');
  const {
    method,
    headers,
    payload
  ...
  if (method !== 'GET' || payload || !_underscore.default.isEmpty(headers)) {
    _log.default.deprecated('Using other request methods than GET, rewriting headers and adding payloads has a high impact on performance ' + 'in recent versions of Puppeteer. Use only when necessary.');
    ...
    const interceptRequestHandler = async interceptedRequest => {
      ...
      if (!_underscore.default.isEmpty(headers)) overrides.headers = headers;
      await interceptedRequest.continue(overrides);
      await (0, _puppeteer_request_interception.removeInterceptRequestHandler)(page, interceptRequestHandler); // We wan't this to be called only for the initial request.
    };

    await (0, _puppeteer_request_interception.addInterceptRequestHandler)(page, interceptRequestHandler);
  }
...
const infiniteScroll = async (page, options = {}) => {
  ...
  (0, _utils.checkParamOrThrow)(page, 'page', 'Object');
  (0, _utils.checkParamOrThrow)(timeoutSecs, 'timeoutSecs', 'Number');
  (0, _utils.checkParamOrThrow)(waitForSecs, 'waitForSecs', 'Number');
  let finished;
...
const saveSnapshot = async (page, options = {}) => {
  ...
  try {
    ...
    const {
      saveScreenshot = true,
      saveHtml = true,
      keyValueStoreName = null,
      screenshotQuality = 50
    } = options;
    key = options.key || DEFAULT_KEY;
    (0, _utils.checkParamOrThrow)(saveScreenshot, 'saveScreenshot', 'Boolean');
    (0, _utils.checkParamOrThrow)(saveHtml, 'saveHtml', 'Boolean');
    (0, _utils.checkParamOrThrow)(key, 'key', 'String');
    (0, _utils.checkParamOrThrow)(keyValueStoreName, 'keyValueStoreName', 'Maybe String');
    (0, _utils.checkParamOrThrow)(screenshotQuality, 'screenshotQuality', 'Number');
    const store = await (0, _key_value_store.openKeyValueStore)(keyValueStoreName);
    ...
    if (saveScreenshot) {
      ...
      await store.setValue(`${key}.jpg`, screenshotBuffer, {
    ...
    if (saveHtml) {
      ...
      await store.setValue(`${key}.html`, html, {
        contentType: 'text/html'
      });
    }
  } catch (e) {
    // I like this more than having to investigate stack trace
    _log.default.error(`saveSnapshot with key ${key || ''} failed with error:`);
...
const puppeteerUtils = {
  ...
  enqueueLinks: async (...args) => {
    if (logEnqueueLinksDeprecationWarning) {
      _log.default.warning('Using enqueueLinks() from the Apify.utils.puppeteer namespace is deprecated. ' + 'Please use the Apify.utils.enqueueLinks().');

      logEnqueueLinksDeprecationWarning = false;
      return (0, _enqueue_links.enqueueLinks)(...args);
    }
  },
  enqueueLinksByClickingElements: _click_elements.enqueueLinksByClickingElements,
  blockRequests,
  blockResources,
  cacheResponses,
  compileScript,
  gotoExtended,
  addInterceptRequestHandler: _puppeteer_request_interception.addInterceptRequestHandler,
  removeInterceptRequestHandler: _puppeteer_request_interception.removeInterceptRequestHandler,
  infiniteScroll,
  saveSnapshot
};
exports.puppeteerUtils = puppeteerUtils;