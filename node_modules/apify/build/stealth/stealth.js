...
var _hiding_tricks = _interopRequireDefault(require("./hiding_tricks"));
...
function applyStealthToBrowser(browser, options) {
  ...
  const opts = _underscore.default.defaults(options, DEFAULT_STEALTH_OPTIONS);
...
function applyStealthTricks(page, options) {
  const functions = Object.keys(options).filter(key => {
    return options[key];
  }).map(key => _hiding_tricks.default[key].toString());
