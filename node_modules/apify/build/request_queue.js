...
exports.openRequestQueue = exports.RequestQueueLocal = exports.RequestQueue = exports.STORAGE_CONSISTENCY_DELAY_MILLIS = exports.MAX_QUERIES_FOR_CONSISTENCY = exports.API_PROCESSED_REQUESTS_DELAY_MILLIS = exports.QUERY_HEAD_BUFFER = exports.QUERY_HEAD_MIN_LENGTH = exports.LOCAL_STORAGE_SUBDIR = void 0;
...
var _lru_cache = _interopRequireDefault(require("apify-shared/lru_cache"));

var _list_dictionary = _interopRequireDefault(require("apify-shared/list_dictionary"));
...
const LOCAL_STORAGE_SUBDIR = _consts.LOCAL_STORAGE_SUBDIRS.requestQueues;
exports.LOCAL_STORAGE_SUBDIR = LOCAL_STORAGE_SUBDIR;
const MAX_OPENED_QUEUES = 1000;
const MAX_CACHED_REQUESTS = 1000 * 1000; // When requesting queue head we always fetch requestsInProgressCount * QUERY_HEAD_BUFFER number of requests.

const QUERY_HEAD_MIN_LENGTH = 100;
exports.QUERY_HEAD_MIN_LENGTH = QUERY_HEAD_MIN_LENGTH;
const QUERY_HEAD_BUFFER = 3; // If queue was modified (request added/updated/deleted) before more than API_PROCESSED_REQUESTS_DELAY_MILLIS
// then we assume the get head operation to be consistent.

exports.QUERY_HEAD_BUFFER = QUERY_HEAD_BUFFER;
const API_PROCESSED_REQUESTS_DELAY_MILLIS = 10 * 1000; // How many times we try to get queue head with queueModifiedAt older than API_PROCESSED_REQUESTS_DELAY_MILLIS.

exports.API_PROCESSED_REQUESTS_DELAY_MILLIS = API_PROCESSED_REQUESTS_DELAY_MILLIS;
const MAX_QUERIES_FOR_CONSISTENCY = 6; // This number must be large enough so that processing of all these requests cannot be done in
// a time lower than expected maximum latency of DynamoDB, but low enough not to waste too much memory.

exports.MAX_QUERIES_FOR_CONSISTENCY = MAX_QUERIES_FOR_CONSISTENCY;
const RECENTLY_HANDLED_CACHE_SIZE = 1000; // Indicates how long it usually takes for the underlying storage to propagate all writes
// to be available to subsequent reads.

const STORAGE_CONSISTENCY_DELAY_MILLIS = 3000;
exports.STORAGE_CONSISTENCY_DELAY_MILLIS = STORAGE_CONSISTENCY_DELAY_MILLIS;
const {
  requestQueues
} = _utils2.apifyClient;
const queuesCache = new _lru_cache.default({
  maxLength: MAX_OPENED_QUEUES
}); // Open queues are stored here.
...
const validateAddRequestParams = (request, opts) => {
  ...
  const newRequest = request instanceof _request.default ? request : new _request.default(request);
  const {
    forefront = false
  } = opts;
  (0, _utils.checkParamOrThrow)(forefront, 'opts.forefront', 'Boolean');
...
const validateGetRequestParams = requestId => {
  (0, _utils.checkParamOrThrow)(requestId, 'requestId', 'String');
};
/**
...
const validateMarkRequestHandledParams = request => {
  (0, _utilities.checkParamPrototypeOrThrow)(request, 'request', _request.default, 'Apify.Request');
  (0, _utils.checkParamOrThrow)(request.id, 'request.id', 'String');
};
/**
...
const getRequestId = uniqueKey => {
  (0, _utils.checkParamOrThrow)(uniqueKey, 'uniqueKey', 'String');

  const str = _crypto.default.createHash('sha256').update(uniqueKey).digest('base64').replace(/(\+|\/|=)/g, '');
...
class RequestQueue {
  constructor(queueId, queueName, clientKey = (0, _utilities.cryptoRandomObjectId)()) {
    (0, _utils.checkParamOrThrow)(queueId, 'queueId', 'String');
    (0, _utils.checkParamOrThrow)(queueName, 'queueName', 'Maybe String');
    (0, _utils.checkParamOrThrow)(clientKey, 'clientKey', 'String');
    if (!clientKey) throw new Error('Parameter "clientKey" must be a non-empty string!');
    this.clientKey = clientKey;
    this.queueId = queueId;
    this.queueName = queueName; // Contains a cached list of request IDs from the head of the queue,
    // as obtained in the last query. Both key and value is the request ID.

    this.queueHeadDict = new _list_dictionary.default();
    this.queryQueueHeadPromise = null; // A set of all request IDs that are currently being handled,
    // i.e. which were returned by fetchNextRequest() but not markRequestHandled()

    this.inProgress = new Set(); // Contains a list of recently handled requests. It is used to avoid inconsistencies
    // caused by delays in the underlying DynamoDB storage.
    // Keys are request IDs, values are true.

    this.recentlyHandled = new _lru_cache.default({
    ...
    this.assumedHandledCount = 0; // Caching requests to avoid redundant addRequest() calls.
    ...
    this.requestsCache = new _lru_cache.default({
  ...
  async addRequest(request, options = {}) {
    const {
      newRequest,
      forefront
    } = validateAddRequestParams(request, options);
    ...
    if (!wasAlreadyPresent && !this.inProgress.has(requestId) && !this.recentlyHandled.get(requestId)) {
      this.assumedTotalCount++; // Performance optimization: add request straight to head if possible
  ...
  async getRequest(requestId) {
    validateGetRequestParams(requestId); // TODO: Could we also use requestsCache here? It would be consistent with addRequest()
    ...
    return obj ? new _request.default(obj) : null;
  }
  /**
  ...
  async fetchNextRequest() {
    ...
    const nextRequestId = this.queueHeadDict.removeFirst(); // We are likely done at this point.

    if (!nextRequestId) return null; // This should never happen, but...

    if (this.inProgress.has(nextRequestId) || this.recentlyHandled.get(nextRequestId)) {
      _log.default.warning('Queue head returned a request that is already in progress?!', {
        nextRequestId,
    ...
    if (!request) {
      _log.default.debug('Cannot find a request from the beginning of queue, will be retried later', {
        nextRequestId
      });
    ...
    if (request.handledAt) {
      _log.default.debug('Request fetched from the beginning of queue was already handled', {
        nextRequestId
      });
  ...
  async reclaimRequest(request, options = {}) {
    ...
    const {
      forefront
    } = validateReclaimRequestParams(request, options);
    ...
    queueOperationInfo.request = request; // Wait a little to increase a chance that the next call to fetchNextRequest() will return the request with updated data.
    // This is to compensate for the limitation of DynamoDB, where writes might not be immediately visible to subsequent reads.

    setTimeout(() => {
      if (!this.inProgress.has(request.id)) {
        _log.default.warning('The request is no longer marked as in progress in the queue?!', {
      ...
      this.inProgress.delete(request.id); // Performance optimization: add request straight to head if possible
  ...
  _cacheRequest(cacheKey, queueOperationInfo) {
    (0, _utils.checkParamOrThrow)(cacheKey, 'cacheKey', 'String');
    (0, _utils.checkParamOrThrow)(queueOperationInfo, 'queueOperationInfo', 'Object');
    (0, _utils.checkParamOrThrow)(queueOperationInfo.requestId, 'queueOperationInfo.requestId', 'String');
    (0, _utils.checkParamOrThrow)(queueOperationInfo.wasAlreadyHandled, 'queueOperationInfo.wasAlreadyHandled', 'Boolean');
  ...
  async _ensureHeadIsNonEmpty(ensureConsistency = false, limit = Math.max(this.inProgressCount() * QUERY_HEAD_BUFFER, QUERY_HEAD_MIN_LENGTH), iteration = 0) {
    (0, _utils.checkParamOrThrow)(ensureConsistency, 'ensureConsistency', 'Boolean');
    (0, _utils.checkParamOrThrow)(limit, 'limit', 'Number');
    (0, _utils.checkParamOrThrow)(iteration, 'iteration', 'Number'); // If is nonempty resolve immediately.

    if (this.queueHeadDict.length() > 0) return true;

    if (!this.queryQueueHeadPromise) {
      const queryStartedAt = new Date();
      this.queryQueueHeadPromise = requestQueues.getHead({
        limit,
        queueId: this.queueId,
        clientKey: this.clientKey
      }).then(({
    ...
    const {
      ...
      queryStartedAt,
      hadMultipleClients
    } = await this.queryQueueHeadPromise; // TODO: I feel this code below can be greatly simplified...
    ...
    if (prevLimit >= _consts.REQUEST_QUEUE_HEAD_MAX_LIMIT) {
      _log.default.warning(`RequestQueue: Reached the maximum number of requests in progress: ${_consts.REQUEST_QUEUE_HEAD_MAX_LIMIT}.`);
    }

    const shouldRepeatWithHigherLimit = this.queueHeadDict.length() === 0 && wasLimitReached && prevLimit < _consts.REQUEST_QUEUE_HEAD_MAX_LIMIT; // If ensureConsistency=true then we must ensure that either:
    ...
    const isLocallyConsistent = !hadMultipleClients && this.assumedTotalCount <= this.assumedHandledCount; // Consistent information from one source is enough to consider request queue finished.

    const shouldRepeatForConsistency = ensureConsistency && !isDatabaseConsistent && !isLocallyConsistent; // If both are false then head is consistent and we may exit.

    if (!shouldRepeatWithHigherLimit && !shouldRepeatForConsistency) return true; // If we are querying for consistency then we limit the number of queries to MAX_QUERIES_FOR_CONSISTENCY.
    ...
    const nextLimit = shouldRepeatWithHigherLimit ? Math.round(prevLimit * 1.5) : prevLimit; // If we are repeating for consistency then wait required time.

    if (shouldRepeatForConsistency) {
      const delayMillis = API_PROCESSED_REQUESTS_DELAY_MILLIS - (Date.now() - queueModifiedAt);

      _log.default.info(`RequestQueue: Waiting for ${delayMillis}ms before considering the queue as finished to ensure that the data is consistent.`);

      await (0, _utils2.sleep)(delayMillis);
    }
  ...
  async delete() {
    _log.default.deprecated('requestQueue.delete() is deprecated. Please use requestQueue.drop() instead. ' + 'This is to make it more obvious to users that the function deletes the request queue and not individual records in the queue.');
...
const filePathToQueueOrderNo = filepath => {
  const int = filepath.split(_path.default.sep).pop() // Get filename from path
...
class RequestQueueLocal {
  constructor(queueId, localStorageDir) {
    (0, _utils.checkParamOrThrow)(queueId, 'queueId', 'String');
    (0, _utils.checkParamOrThrow)(localStorageDir, 'localStorageDir', 'String');
    this.queueId = queueId;
    this.localStoragePath = _path.default.resolve(_path.default.join(localStorageDir, LOCAL_STORAGE_SUBDIR, queueId));
    this.localHandledEmulationPath = _path.default.join(this.localStoragePath, 'handled');
    this.localPendingEmulationPath = _path.default.join(this.localStoragePath, 'pending');
  ...
  async _initialize() {
    ...
    await (0, _utils2.ensureDirExists)(this.localHandledEmulationPath);
    await (0, _utils2.ensureDirExists)(this.localPendingEmulationPath);
    const [handled, pending] = await Promise.all([_fsExtra.default.readdir(this.localHandledEmulationPath), _fsExtra.default.readdir(this.localPendingEmulationPath)]);
    this.pendingCount = pending.length;
    this._handledCount = handled.length;
    const handledPaths = handled.map(filename => _path.default.join(this.localHandledEmulationPath, filename));
    const pendingPaths = pending.map(filename => _path.default.join(this.localPendingEmulationPath, filename));
    const filePaths = handledPaths.concat(pendingPaths);
    const stats = await _fsExtra.default.stat(this.localPendingEmulationPath);
  ...
  async _saveRequestIdToQueueOrderNo(filepath) {
    const str = await _fsExtra.default.readFile(filepath);
  ...
  _getFilePath(queueOrderNo, isHandled = false) {
    ...
    const dir = isHandled ? this.localHandledEmulationPath : this.localPendingEmulationPath;
  ...
  _getQueueOrderNo(forefront = false) {
    ...
    return forefront ? sgn + (base - now) : sgn + (base + now);
  }

  async _getRequestByQueueOrderNo(queueOrderNo) {
    (0, _utils.checkParamOrThrow)(queueOrderNo, 'queueOrderNo', 'Number');
    let buffer;
    let filePath;

    try {
      filePath = this._getFilePath(queueOrderNo);
      buffer = await _fsExtra.default.readFile(filePath);
    } catch (err) {
    ...
    return new _request.default(requestOptions);
  }
  ...
  async addRequest(request, opts = {}) {
    const {
      newRequest,
      forefront
    } = validateAddRequestParams(request, opts);
    request.uniqueKey = newRequest.uniqueKey;
    await this.initializationPromise;

    const queueOrderNo = this._getQueueOrderNo(forefront); // Add ID as server does.
    ...
    this._updateMetadata(true); // If request already exists then don't override it!
    ...
    const filePath = this._getFilePath(queueOrderNo, !!requestCopy.handledAt); // Lock the file until we fully dump data!
    ...
    await _fsExtra.default.writeFile(filePath, JSON.stringify(requestCopy, null, 4));
  ...
  async getRequest(requestId) {
    ...
    this._updateMetadata(); // TODO: We should update the last access time to files...
  ...
  async fetchNextRequest() {
    await this.initializationPromise;
    const files = await _fsExtra.default.readdir(this.localPendingEmulationPath);
    ...
    while (!request && files.length) {
      ...
      this.inProgressCount++; // TODO: There must be a better way. This try/catch is here because there is a race condition between
  ...
  async markRequestHandled(request) {
    ...
    this._updateMetadata(true); // NOTE: First write to old file and then rename to new one to do the operation atomically.
    //       Situation where two files exists at the same time may cause race condition bugs.


    await _fsExtra.default.writeFile(source, JSON.stringify(request, null, 4));
    await _fsExtra.default.rename(source, dest);
    this.pendingCount--;
    this._handledCount++;
    this.inProgressCount--;
    delete this.queueOrderNoInProgress[queueOrderNo];
  ...
  async reclaimRequest(request, opts = {}) {
    const {
      forefront
    } = validateReclaimRequestParams(request, opts);
    await this.initializationPromise;
  ...
  async drop() {
    await _fsExtra.default.emptyDir(this.localStoragePath);
    queuesCache.remove(this.queueId);
  }
  ...
  async getInfo() {
    ...
    const name = id === _consts.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID ? null : id;
    const result = {
      id,
      name,
...
exports.RequestQueueLocal = RequestQueueLocal;

const getOrCreateQueue = async queueIdOrName => {
  const existingQueue = await requestQueues.getQueue({
    queueId: queueIdOrName
  });
  if (existingQueue) return existingQueue;
  return requestQueues.getOrCreateQueue({
...
const openRequestQueue = (queueIdOrName, options = {}) => {
  (0, _utils.checkParamOrThrow)(queueIdOrName, 'queueIdOrName', 'Maybe String');
  (0, _utils.checkParamOrThrow)(options, 'options', 'Object');
  (0, _utils2.ensureTokenOrLocalStorageEnvExists)('request queue');
  const {
    forceCloud = false
  } = options;
  (0, _utils.checkParamOrThrow)(forceCloud, 'options.forceCloud', 'Boolean');
  return process.env[_consts.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud ? (0, _utils2.openLocalStorage)(queueIdOrName, _consts.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID, RequestQueueLocal, queuesCache) : (0, _utils2.openRemoteStorage)(queueIdOrName, _consts.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID, RequestQueue, queuesCache, getOrCreateQueue);
};
