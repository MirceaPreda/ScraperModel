...
var _stealth = _interopRequireDefault(require("./stealth/stealth"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const LAUNCH_PUPPETEER_LOG_OMIT_OPTS = ['proxyUrl', 'userAgent', 'useApifyProxy', 'apifyProxySession', 'apifyProxyGroups'];
...
const launchPuppeteerWithProxy = async (puppeteer, opts) => {
  ...
  const parsedProxyUrl = (0, _proxyChain.parseUrl)(opts.proxyUrl);
  ...
  if (parsedProxyUrl.username || parsedProxyUrl.password) {
    ...
    anonymizedProxyUrl = await (0, _proxyChain.anonymizeProxy)(opts.proxyUrl);
  }

  opts.args.push(`--proxy-server=${anonymizedProxyUrl || opts.proxyUrl}`);

  const optsForLog = _underscore.default.omit(opts, LAUNCH_PUPPETEER_LOG_OMIT_OPTS);

  optsForLog.proxyUrl = (0, _proxyChain.redactUrl)(opts.proxyUrl);
  optsForLog.args = opts.args.slice(0, opts.args.length - 1);

  _log.default.info('Launching Puppeteer', optsForLog);

  const browser = await puppeteer.launch(opts); // Close anonymization proxy server when Puppeteer finishes

  if (anonymizedProxyUrl) {
    const cleanUp = () => {
      // Don't wait for finish, only log errors
      (0, _proxyChain.closeAnonymizedProxy)(anonymizedProxyUrl, true).catch(err => _log.default.exception(err, 'closeAnonymizedProxy() failed.'));
    };
...
const getPuppeteerOrThrow = (puppeteerModule = 'puppeteer') => {
  (0, _utils.checkParamOrThrow)(puppeteerModule, 'puppeteerModule', 'String|Object');
  ...
  try {
    ...
    if (err.code === 'MODULE_NOT_FOUND') {
      ...
      err.message = (0, _utils2.isAtHome)() ? `${msg} The 'puppeteer' package is automatically bundled when using apify/actor-node-chrome-* Base image.` : msg;
    }
...
const launchPuppeteer = async (options = {}) => {
  (0, _utils.checkParamOrThrow)(options, 'options', 'Object');
  (0, _utils.checkParamOrThrow)(options.args, 'options.args', 'Maybe [String]');
  (0, _utils.checkParamOrThrow)(options.proxyUrl, 'options.proxyUrl', 'Maybe String');
  (0, _utils.checkParamOrThrow)(options.useApifyProxy, 'options.useApifyProxy', 'Maybe Boolean');
  (0, _utils.checkParamOrThrow)(options.puppeteerModule, 'options.puppeteerModule', 'Maybe String|Object');
  (0, _utils.checkParamOrThrow)(options.stealth, 'options.stealth', 'Maybe Boolean');
  (0, _utils.checkParamOrThrow)(options.stealthOptions, 'options.stealthOptions', 'Maybe Object');
  ...
  if (options.liveView || options.liveViewOptions) {
    _log.default.deprecated('Live view is no longer available in Apify.launchPuppeteer() and launchPuppeteerOptions. ' + 'Use options.useLiveView in PuppeteerPool for an updated version. ' + 'For live view with Apify.launchPuppeteer(), use Apify.LiveViewServer.');
  }
  ...
  if (optsCopy.headless == null) {
    optsCopy.headless = process.env[_consts.ENV_VARS.HEADLESS] === '1' && process.env[_consts.ENV_VARS.XVFB] !== '1';
  }

  if (optsCopy.useChrome && (optsCopy.executablePath === undefined || optsCopy.executablePath === null)) {
    optsCopy.executablePath = process.env[_consts.ENV_VARS.CHROME_EXECUTABLE_PATH] || (0, _utils2.getTypicalChromeExecutablePath)();
  }

  if (optsCopy.useApifyProxy) {
    optsCopy.proxyUrl = (0, _actor.getApifyProxyUrl)({
  ...
  if (!userAgent && (!optsCopy.executablePath || optsCopy.headless)) {
    userAgent = _constants.DEFAULT_USER_AGENT;
  }
  ...
  if (optsCopy.proxyUrl) {
    ...
    _log.default.info('Launching Puppeteer', _underscore.default.omit(optsCopy, LAUNCH_PUPPETEER_LOG_OMIT_OPTS));
  ...
  if (optsCopy.stealth) {
    browser = (0, _stealth.default)(browser, optsCopy.stealthOptions);
  }
