 ...
 * @hideconstructor
 */
class SessionPool extends _events.default {
  /**
  ...
  constructor(options = {}) {
    ...
    (0, _utils.checkParamOrThrow)(maxPoolSize, 'options.maxPoolSize', 'Number');
    (0, _utils.checkParamOrThrow)(sessionOptions, 'options.sessionOptions', 'Object');
    (0, _utils.checkParamOrThrow)(persistStateKeyValueStoreId, 'options.persistStateKeyValueStoreId', 'Maybe String');
    (0, _utils.checkParamOrThrow)(persistStateKey, 'options.persistStateKey', 'String');
    (0, _utils.checkParamOrThrow)(createSessionFunction, 'options.createSessionFunction', 'Maybe Function'); // Pool Configuration

    this.maxPoolSize = maxPoolSize;
    this.createSessionFunction = createSessionFunction || this._defaultCreateSessionFunction; // Session configuration

    this.sessionOptions = sessionOptions; // Session keyValueStore

    this.persistStateKeyValueStoreId = persistStateKeyValueStoreId;
    this.persistStateKey = persistStateKey; // Operative states
  ...
  async initialize() {
    this.keyValueStore = await (0, _key_value_store.openKeyValueStore)(this.persistStateKeyValueStoreId); // in case of migration happened and SessionPool state should be restored from the keyValueStore.

    await this._maybeLoadSessionPool();
    this._listener = this.persistState.bind(this);

    _events2.default.on(_constants.ACTOR_EVENT_NAMES_EX.PERSIST_STATE, this._listener);
  }
  /**
   * Gets session.
  ...
  async persistState() {
    _log.default.debug('SessionPool: Persisting state', {
  ...
  teardown() {
    _events2.default.removeListener(_constants.ACTOR_EVENT_NAMES_EX.PERSIST_STATE, this._listener);
  }
  /**
  ...
  _removeSession(session) {
    ...
    _log.default.debug(`SessionPool: Removed Session - ${removedSession.id}`);
  }
  /**
  ...
  _defaultCreateSessionFunction(sessionPool) {
    return new _session.Session({ ...this.sessionOptions,
      sessionPool
    });
  }
  /**
  ...
  async _createSession() {
    ...
    _log.default.debug(`SessionPool: Created new Session - ${newSession.id}`);
  ...
  async _maybeLoadSessionPool() {
    ...
    if (!loadedSessionPool) return; // Invalidate old sessions and load active sessions only

    _log.default.debug('SessionPool: Recreating state from KeyValueStore', {
      persistStateKeyValueStoreId: this.persistStateKeyValueStoreId,
      persistStateKey: this.persistStateKey
    });
    ...
    for (const sessionObject of loadedSessionPool.sessions) {
      ...
      const recreatedSession = new _session.Session(sessionObject);
    ...
    _log.default.debug(`SessionPool: ${this.usableSessionsCount} active sessions loaded from KeyValueStore`);
  }
