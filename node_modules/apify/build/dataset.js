...
exports.pushData = exports.openDataset = exports.DatasetLocal = exports.Dataset = exports.chunkBySize = exports.checkAndSerialize = exports.LOCAL_GET_ITEMS_DEFAULT_LIMIT = exports.LOCAL_FILENAME_DIGITS = exports.LOCAL_STORAGE_SUBDIR = exports.DATASET_ITERATORS_DEFAULT_LIMIT = void 0;
...
const DATASET_ITERATORS_DEFAULT_LIMIT = 10000;
exports.DATASET_ITERATORS_DEFAULT_LIMIT = DATASET_ITERATORS_DEFAULT_LIMIT;
const LOCAL_STORAGE_SUBDIR = _consts.LOCAL_STORAGE_SUBDIRS.datasets;
exports.LOCAL_STORAGE_SUBDIR = LOCAL_STORAGE_SUBDIR;
const LOCAL_FILENAME_DIGITS = 9;
exports.LOCAL_FILENAME_DIGITS = LOCAL_FILENAME_DIGITS;
const LOCAL_GET_ITEMS_DEFAULT_LIMIT = 250000;
exports.LOCAL_GET_ITEMS_DEFAULT_LIMIT = LOCAL_GET_ITEMS_DEFAULT_LIMIT;
...
const getLocaleFilename = index => `${(0, _utilities.leftpad)(index, LOCAL_FILENAME_DIGITS, 0)}.json`;

const {
  datasets
} = _utils2.apifyClient;
const datasetsCache = new _lru_cache.default({
  maxLength: MAX_OPENED_STORES
}); // Open Datasets are stored here.
...
const checkAndSerialize = (item, limitBytes, index) => {
  ...
  try {
    (0, _utils.checkParamOrThrow)(item, 'item', 'Object');
...
const chunkBySize = (items, limitBytes) => {
  ...
  const chunks = []; // Split payloads into buckets of valid size.
  ...
  for (const payload of items) {
    ...
    if (bytes <= limitBytes && bytes + 2 > limitBytes) {
      ...
      if (!Array.isArray(_underscore.default.last(chunks))) chunks.push([]); // ensure array

      _underscore.default.last(chunks).push(payload);
...
class Dataset {
  constructor(datasetId, datasetName) {
    ...
    (0, _utils.checkParamOrThrow)(datasetName, 'datasetName', 'Maybe String');
  ...
  async pushData(data) {
    (0, _utils.checkParamOrThrow)(data, 'data', 'Array | Object');

    const dispatch = async payload => datasets.putItems({
      datasetId: this.datasetId,
      data: payload
    });

    const limit = _consts.MAX_PAYLOAD_SIZE_BYTES - Math.ceil(_consts.MAX_PAYLOAD_SIZE_BYTES * SAFETY_BUFFER_PERCENT); // Handle singular Objects
    ...
    const chunks = chunkBySize(payloads, limit); // Invoke client in series to preserve order of data
  ...
  map(iteratee, options) {
    ...
    const wrappedFunc = (item, index) => {
      return Promise.resolve().then(() => iteratee(item, index)).then(res => result.push(res));
    };
  ...
  async delete() {
    _log.default.deprecated('dataset.delete() is deprecated. Please use dataset.drop() instead. ' + 'This is to make it more obvious to users that the function deletes the dataset and not individual records in the dataset.');
...
class DatasetLocal {
  constructor(datasetId, localStorageDir) {
    ...
    this.localStoragePath = _path.default.resolve(_path.default.join(localStorageDir, LOCAL_STORAGE_SUBDIR, datasetId));
    this.counter = null;
    this.datasetId = datasetId;
    this.createdAt = null;
  ...
  _initialize() {
    return (0, _utils2.ensureDirExists)(this.localStoragePath).then(() => readdirPromised(this.localStoragePath)).then(files => {
      if (files.length) {
  ...
  pushData(data) {
    (0, _utils.checkParamOrThrow)(data, 'data', 'Array | Object');
    if (!_underscore.default.isArray(data)) data = [data];
    return this.initializationPromise.then(() => {
      const promises = data.map(item => {
        this.counter++; // Format JSON to simplify debugging, the overheads is negligible

        const itemStr = JSON.stringify(item, null, 2);

        const filePath = _path.default.join(this.localStoragePath, getLocaleFilename(this.counter));
  ...
  async getData(opts = {}) {
    ...
    (0, _utils.checkParamOrThrow)(opts.limit, 'opts.limit', 'Maybe Number');
    (0, _utils.checkParamOrThrow)(opts.offset, 'opts.offset', 'Maybe Number');
    (0, _utils.checkParamOrThrow)(opts.desc, 'opts.desc', 'Maybe Boolean');
  ...
  async getInfo() {
    ...
    const name = id === _consts.ENV_VARS.DEFAULT_DATASET_ID ? null : id;
    const result = {
      id,
      name,
  ...
  _getItemIndexes(offset = 0, limit = this.counter) {
    ...
    if (start > end) return [];
    return _underscore.default.range(start, end);
  }
  /**
  ...
  _readAndParseFile(index) {
    const filePath = _path.default.join(this.localStoragePath, getLocaleFilename(index));

    return readFilePromised(filePath).then(json => {
      this._updateMetadata();
...
const openDataset = (datasetIdOrName, options = {}) => {
  (0, _utils.checkParamOrThrow)(datasetIdOrName, 'datasetIdOrName', 'Maybe String');
  (0, _utils.checkParamOrThrow)(options, 'options', 'Object');
  (0, _utils2.ensureTokenOrLocalStorageEnvExists)('dataset');
  const {
    forceCloud = false
  } = options;
  (0, _utils.checkParamOrThrow)(forceCloud, 'options.forceCloud', 'Boolean');
  return process.env[_consts.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud ? (0, _utils2.openLocalStorage)(datasetIdOrName, _consts.ENV_VARS.DEFAULT_DATASET_ID, DatasetLocal, datasetsCache) : (0, _utils2.openRemoteStorage)(datasetIdOrName, _consts.ENV_VARS.DEFAULT_DATASET_ID, Dataset, datasetsCache, getOrCreateDataset);
};
/**
...
const pushData = item => openDataset().then(dataset => dataset.pushData(item));
