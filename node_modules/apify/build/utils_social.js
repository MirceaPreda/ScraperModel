...
const emailsFromText = text => {
  if (!_underscore.default.isString(text)) return [];
...
const PHONE_URL_PREFIX_REGEX = /^(tel|phone|telephone|callto):(\/)?(\/)?/i; // It's pretty much impossible (and unmaintainable) to have just one large regular expression for all possible phone numbers.
...
const PHONE_REGEXS_STRINGS = [// 775123456
...
'([0-9]{1,4}( )?)?\\([0-9]{2,4}\\)( )?[0-9]{2,4}(( )?(-|.))?( )?[0-9]{2,6}', // 413-577-1234-564
'[0-9]{2,4}-[0-9]{2,4}-[0-9]{2,4}-[0-9]{2,6}', // 413-577-1234
'[0-9]{2,4}-[0-9]{2,4}-[0-9]{2,6}', // 413-577
'[0-9]{2,4}-[0-9]{2,6}', // 413.577.1234.564
'[0-9]{2,4}\\.[0-9]{2,4}\\.[0-9]{2,4}\\.[0-9]{2,6}', // 413.577.1234
'[0-9]{2,4}\\.[0-9]{2,4}\\.[0-9]{2,6}', // 413.577
'[0-9]{2,4}\\.[0-9]{2,6}', // 413 577 1234 564
'[0-9]{2,4} [0-9]{2,4} [0-9]{2,4} [0-9]{2,6}', // 413 577 1234
'[0-9]{2,4} [0-9]{2,4} [0-9]{2,6}', // 123 4567
'[0-9]{2,4} [0-9]{3,8}']; // All phones might be prefixed with '+' or '00'
...
const PHONE_MIN_DIGITS = 7; // These are patterns that might be matched by PHONE_REGEXS_STRINGS,
// but which are most likely not phone numbers. Add more patterns as needed.

const SKIP_PHONE_REGEXS = [// 2018-11-10
'^[0-9]{4}-[0-9]{2}-[0-9]{2}$'];
...
const phonesFromText = text => {
  ...
  phones = phones.filter(phone => {
    if (!phone) return false; // Skip too short phones, they are most likely incorrect

    if (phone.match(/[0-9]/g).length < PHONE_MIN_DIGITS) return false; // Skip phone numbers matching specific patterns
...
const LINKEDIN_REGEX_STRING = '(?<!\\w)(?:http(?:s)?:\\/\\/)?(?:(?:[a-z]+\\.)?linkedin\\.com\\/in\\/)([a-z0-9\\-_%]{2,60})(?![a-z0-9\\-_%])(?:/)?'; // TODO: Skip https://www.instagram.com/explore/ !!! and "https://www.instagram.com/_n/", "https://www.instagram.com/_u/"
// eslint-disable-next-line max-len

const INSTAGRAM_REGEX_STRING = '(?<!\\w)(?:http(?:s)?:\\/\\/)?(?:(?:www\\.)?(?:instagram\\.com|instagr\\.am)\\/)([a-z0-9_.]{2,30})(?![a-z0-9_.])(?:/)?';
const TWITTER_RESERVED_PATHS = 'oauth|account|tos|privacy|signup|home|hashtag|search|login|widgets|i|settings|start|share|intent|oct'; // eslint-disable-next-line max-len, quotes

const TWITTER_REGEX_STRING = `(?<!\\w)(?:http(?:s)?:\\/\\/)?(?:www.)?(?:twitter.com)\\/(?!(?:${TWITTER_RESERVED_PATHS})(?:[\\'\\"\\?\\.\\/]|$))([a-z0-9_]{1,15})(?![a-z0-9_])(?:/)?`; // eslint-disable-next-line max-len, quotes

const FACEBOOK_RESERVED_PATHS = 'rsrc\\.php|apps|groups|events|l\\.php|friends|images|photo.php|chat|ajax|dyi|common|policies|login|recover|reg|help|security|messages|marketplace|pages|live|bookmarks|games|fundraisers|saved|gaming|salesgroups|jobs|people|ads|ad_campaign|weather|offers|recommendations|crisisresponse|onthisday|developers|settings|connect|business|plugins|intern|sharer'; // eslint-disable-next-line max-len, quotes

const FACEBOOK_REGEX_STRING = `(?<!\\w)(?:http(?:s)?:\\/\\/)?(?:www.)?(?:facebook.com|fb.com)\\/(?!(?:${FACEBOOK_RESERVED_PATHS})(?:[\\'\\"\\?\\.\\/]|$))(profile\\.php\\?id\\=[0-9]{3,20}|(?!profile\\.php)[a-z0-9\\.]{5,51})(?![a-z0-9\\.])(?:/)?`; // eslint-disable-next-line max-len, quotes
...
try {
  ...
  if (e && e.message && e.message.includes('Invalid group')) {
    // eslint-disable-next-line max-len
    _log.default.warning(`Your version of Node.js (${process.version}) doesn't support the regular expression syntax used by Apify.utils.social tools. The tools will not work. Please upgrade your Node.js to the latest version.`);
  } else {
    throw e;
  }
}
...
const parseHandlesFromHtml = (html, data = null) => {
  ...
  if (!_underscore.default.isString(html)) return result;
  ...
  const text = _utils.publicUtils.htmlToText($);

  if (data) data.text = text; // Find all <a> links with href tag
  ...
  result.phonesUncertain = phonesFromText(text); // Note that these regexps extract just the base profile path. For example for
  ...
  result.youtubes = html.match(YOUTUBE_REGEX_GLOBAL) || []; // Sort and deduplicate handles
  // eslint-disable-next-line guard-for-in, no-restricted-syntax

  for (const key in result) {
    result[key].sort();
    result[key] = _underscore.default.uniq(result[key], true);
  }
