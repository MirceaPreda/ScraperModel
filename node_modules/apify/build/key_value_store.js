...
exports.getInput = exports.setValue = exports.getValue = exports.openKeyValueStore = exports.KeyValueStoreLocal = exports.getFileNameRegexp = exports.KeyValueStore = exports.maybeStringify = exports.LOCAL_STORAGE_SUBDIR = void 0;
...
const LOCAL_STORAGE_SUBDIR = _consts.LOCAL_STORAGE_SUBDIRS.keyValueStores;
...
const readFilePromised = (0, _util.promisify)(_fsExtra.default.readFile);
const readdirPromised = (0, _util.promisify)(_fsExtra.default.readdir);
const writeFilePromised = (0, _util.promisify)(_fsExtra.default.writeFile);
const unlinkPromised = (0, _util.promisify)(_fsExtra.default.unlink);
const statPromised = (0, _util.promisify)(_fsExtra.default.stat);
const emptyDirPromised = (0, _util.promisify)(_fsExtra.default.emptyDir);
const {
  keyValueStores
} = _utils2.apifyClient;
const storesCache = new _lru_cache.default({
  maxLength: MAX_OPENED_STORES
}); // Open key-value stores are stored here.
...
const validateSetValueParams = (key, value, options) => {
  ...
  (0, _utils.checkParamOrThrow)(options.contentType, 'options.contentType', 'String | Null | Undefined');
  ...
  if (options.contentType) {
    (0, _utils.checkParamOrThrow)(value, 'value', 'Buffer | String', 'The "value" parameter must be a String or Buffer when "options.contentType" is specified.'); // eslint-disable-line max-len
  }

  if (options.contentType === '') throw new Error('Parameter options.contentType cannot be empty string.');
  if (!key) throw new Error('The "key" parameter cannot be empty');

  if (!_regexs.KEY_VALUE_STORE_KEY_REGEX.test(key)) {
    throw new Error('The "key" parameter must be at most 256 characters long and only contain the following characters: ' + "a-zA-Z0-9!-_.'()");
  }
};
/**
...
const maybeStringify = (value, options) => {
  ...
  if (options.contentType === null || options.contentType === undefined) {
    ...
    try {
      ...
      value = (0, _utilities.jsonStringifyExtended)(value, null, 2);
    } catch (e) {
    ...
    if (value === undefined) {
      throw new Error('The "value" parameter was stringified to JSON and returned undefined. ' + 'Make sure you\'re not trying to stringify an undefined value.');
    }
  }
...
class KeyValueStore {
  constructor(storeId, storeName) {
    ...
    (0, _utils.checkParamOrThrow)(storeName, 'storeName', 'Maybe String');
  ...
  getValue(key) {
    validateGetValueParams(key); // TODO: Perhaps we should add options.contentType or options.asBuffer/asString
  ...
  setValue(key, value, options = {}) {
    validateSetValueParams(key, value, options); // Make copy of options, don't update what user passed.

    const optionsCopy = Object.assign({}, options); // In this case delete the record.

    if (value === null) return keyValueStores.deleteRecord({
      storeId: this.storeId,
      key
    });
    value = maybeStringify(value, optionsCopy); // Keep this code in main scope so that simple errors are thrown rather than rejected promise.

    return keyValueStores.putRecord({
      storeId: this.storeId,
      key,
      body: value,
      contentType: (0, _utils2.addCharsetToContentType)(optionsCopy.contentType)
    });
  }
  /**
  ...
  async drop() {
    await keyValueStores.deleteStore({
      storeId: this.storeId
    });
  ...
  async delete() {
    _log.default.deprecated('keyValueStore.delete() is deprecated. Please use keyValueStore.drop() instead. ' + 'This is to make it more obvious to users that the function deletes the key-value store and not individual records in the store.');
  ...
  getPublicUrl(key) {
    return `${_constants.APIFY_API_BASE_URL}/key-value-stores/${this.storeId}/records/${key}`;
  }
  /**
  ...
  async forEachKey(iteratee, options = {}, index = 0) {
    ...
    (0, _utils.checkParamOrThrow)(iteratee, 'iteratee', 'Function');
    (0, _utils.checkParamOrThrow)(exclusiveStartKey, 'options.exclusiveStartKey', 'Maybe String');
    (0, _utils.checkParamOrThrow)(index, 'index', 'Number');
    const response = await keyValueStores.listKeys({
      storeId: this.storeId,
      exclusiveStartKey
    });
    const {
    ...
    for (const item of items) {
      await iteratee(item.key, index++, {
        size: item.size
      });
    }

    return isTruncated ? this.forEachKey(iteratee, {
...
exports.getFileNameRegexp = getFileNameRegexp;

class KeyValueStoreLocal {
  constructor(storeId, localStorageDir) {
    (0, _utils.checkParamOrThrow)(storeId, 'storeId', 'String');
    (0, _utils.checkParamOrThrow)(localStorageDir, 'localStorageDir', 'String');
    this.localStoragePath = _path.default.resolve(_path.default.join(localStorageDir, LOCAL_STORAGE_SUBDIR, storeId));
    this.storeId = storeId;
    this.initializationPromise = (0, _utils2.ensureDirExists)(this.localStoragePath);
  }
  ...
  async getValue(key) {
    ...
    try {
      ...
      return result ? (0, _utils.parseBody)(result.returnValue, _mimeTypes.default.contentType(result.fileName)) : null;
    } catch (err) {
      throw new Error(`Error reading file '${key}' in directory '${this.localStoragePath}' referred by ${_consts.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line
    }
  }

  async setValue(key, value, options = {}) {
    validateSetValueParams(key, value, options);
    await this.initializationPromise; // Make copy of options, don't update what user passed.

    const optionsCopy = Object.assign({}, options); // First remove original file.

    try {
      await this._handleFile(key, unlinkPromised);
    } catch (err) {
      throw new Error(`Error removing file '${key}' in directory '${this.localStoragePath}' referred by ${_consts.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line max-len
    ...
    const contentType = _contentType.default.parse(optionsCopy.contentType).type;

    const extension = _mimeTypes.default.extension(contentType) || DEFAULT_LOCAL_FILE_EXTENSION;
    ...
    try {
      ...
      throw new Error(`Error writing file '${key}' in directory '${this.localStoragePath}' referred by ${_consts.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line max-len
    }
  }
  ...
  async forEachKey(iteratee, options = {}, index = 0) {
    ...
    for (const file of files) {
      try {
        const {
          size
        } = await statPromised(this._getPath(file));
        keys.push({
          key: _path.default.parse(file).name,
          info: {
            size
          }
        });
  ...
  _fullDirectoryLookup(key, handler) {
    return readdirPromised(this.localStoragePath).then(files => {
      const regex = getFileNameRegexp(key);
      const fileName = files.find(file => regex.test(file));
      return fileName ? handler(this._getPath(fileName)).then(returnValue => ({
        returnValue,
        fileName
      })) : null;
    });
  ...
  _getPath(fileName) {
    return _path.default.resolve(this.localStoragePath, fileName);
  }
  /**
...
exports.KeyValueStoreLocal = KeyValueStoreLocal;

const getOrCreateKeyValueStore = storeIdOrName => {
  return _utils2.apifyClient.keyValueStores.getStore({
    storeId: storeIdOrName
  }).then(existingStore => {
    if (existingStore) return existingStore;
    return _utils2.apifyClient.keyValueStores.getOrCreateStore({
...
const openKeyValueStore = (storeIdOrName, options = {}) => {
  (0, _utils.checkParamOrThrow)(storeIdOrName, 'storeIdOrName', 'Maybe String');
  (0, _utils.checkParamOrThrow)(options, 'options', 'Object');
  (0, _utils2.ensureTokenOrLocalStorageEnvExists)('key value store');
  const {
    forceCloud = false
  } = options;
  (0, _utils.checkParamOrThrow)(forceCloud, 'options.forceCloud', 'Boolean');
  return process.env[_consts.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud ? (0, _utils2.openLocalStorage)(storeIdOrName, _consts.ENV_VARS.DEFAULT_KEY_VALUE_STORE_ID, KeyValueStoreLocal, storesCache) : (0, _utils2.openRemoteStorage)(storeIdOrName, _consts.ENV_VARS.DEFAULT_KEY_VALUE_STORE_ID, KeyValueStore, storesCache, getOrCreateKeyValueStore);
};
/**
...
const getInput = async () => getValue(process.env[_consts.ENV_VARS.INPUT_KEY] || _consts.KEY_VALUE_STORE_KEYS.INPUT);
