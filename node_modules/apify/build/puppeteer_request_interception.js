...
const handleRequest = (request, interceptRequestHandlers) => {
  ...
  request.continue = (overrides = {}) => {
    ...
    Object.assign(accumulatedOverrides, overrides, {
      headers
    });
  };

  request.abort = _underscore.default.wrap(request.abort.bind(request), (abort, ...args) => {
    wasAborted = true;
    return abort(...args);
  });
  request.respond = _underscore.default.wrap(request.respond.bind(request), (respond, ...args) => {
    wasResponded = true;
    return respond(...args);
  });

  _underscore.default.some(interceptRequestHandlers, handler => {
    wasContinued = false;
    handler(request); // Check that one of the functions was called.
...
const addInterceptRequestHandler = async (page, handler) => {
  ...
  (0, _utils.checkParamOrThrow)(handler, 'handler', 'Function'); // We haven't initiated an array of handlers yet.
  ...
  handlersArray.push(handler); // First handler was just added at this point so we need to set up request interception.

  if (handlersArray.length === 1) {
    await page.setRequestInterception(true); // This is a handler that get's set in page.on('request', ...) and that executes all the user
...
const removeInterceptRequestHandler = async (page, handler) => {
  ...
  pageInterceptRequestHandlersMap.set(page, handlersArray); // There are no more handlers so we can turn off request interception and remove master handler.
