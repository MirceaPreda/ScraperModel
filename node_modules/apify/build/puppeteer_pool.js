...
exports.default = exports.BROWSER_SESSION_KEY_NAME = void 0;
...
var _linked_list = _interopRequireDefault(require("apify-shared/linked_list"));
...
var _events = _interopRequireDefault(require("./session_pool/events"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const BROWSER_SESSION_KEY_NAME = 'APIFY_SESSION';
exports.BROWSER_SESSION_KEY_NAME = BROWSER_SESSION_KEY_NAME;
...
const DEFAULT_OPTIONS = {
  ...
  launchPuppeteerFunction: launchPuppeteerOptions => (0, _puppeteer.launchPuppeteer)(launchPuppeteerOptions),
  recycleDiskCache: false,
  useIncognitoPages: false,
  useLiveView: false
};

const mkdtempAsync = _util.default.promisify(_fs.default.mkdtemp);

const rimrafAsync = _util.default.promisify(_rimraf.default);

const DISK_CACHE_DIR = _path.default.join(_os.default.tmpdir(), 'puppeteer_disk_cache-');
/**
...
const deleteDiskCacheDir = diskCacheDir => {
  _log.default.debug('PuppeteerPool: Deleting disk cache directory', {
    diskCacheDir
  });

  return rimrafAsync(diskCacheDir).catch(err => {
    _log.default.warning('PuppeteerPool: Cannot delete Chrome disk cache directory', {
      diskCacheDir,
...
class PuppeteerPool {
  constructor(options = {}) {
    const {
      ...
      useLiveView,
      sessionPool
    } = _underscore.default.defaults({}, options, DEFAULT_OPTIONS); // Disabling due to memory leak.


    const reusePages = false;
    (0, _utils.checkParamOrThrow)(reusePages, 'options.reusePages', 'Boolean');
    (0, _utils.checkParamOrThrow)(maxOpenPagesPerInstance, 'options.maxOpenPagesPerInstance', 'Number');
    (0, _utils.checkParamOrThrow)(retireInstanceAfterRequestCount, 'options.retireInstanceAfterRequestCount', 'Number');
    (0, _utils.checkParamOrThrow)(launchPuppeteerFunction, 'options.launchPuppeteerFunction', 'Function');
    (0, _utils.checkParamOrThrow)(puppeteerOperationTimeoutSecs, 'options.puppeteerOperationTimeoutSecs', 'Number');
    (0, _utils.checkParamOrThrow)(instanceKillerIntervalMillis, 'options.instanceKillerIntervalMillis', 'Maybe Number');

    if (instanceKillerIntervalMillis) {
      _log.default.deprecated('PuppeteerPool: options.instanceKillerIntervalMillis is deprecated, use options.instanceKillerIntervalSecs instead.');
    }

    (0, _utils.checkParamOrThrow)(instanceKillerIntervalSecs, 'options.instanceKillerIntervalSecs', 'Number');
    (0, _utils.checkParamOrThrow)(killInstanceAfterMillis, 'options.killInstanceAfterMillis', 'Maybe Number');

    if (killInstanceAfterMillis) {
      _log.default.deprecated('PuppeteerPool: options.killInstanceAfterMillis is deprecated, use options.killInstanceAfterSecs instead.');
    }

    (0, _utils.checkParamOrThrow)(killInstanceAfterSecs, 'options.killInstanceAfterSecs', 'Number');
    (0, _utils.checkParamOrThrow)(launchPuppeteerOptions, 'options.launchPuppeteerOptions', 'Maybe Object');
    (0, _utils.checkParamOrThrow)(recycleDiskCache, 'options.recycleDiskCache', 'Maybe Boolean');
    (0, _utils.checkParamOrThrow)(useIncognitoPages, 'options.useIncognitoPages', 'Maybe Boolean');
    (0, _utils.checkParamOrThrow)(proxyUrls, 'options.proxyUrls', 'Maybe Array'); // Enforce non-empty proxyUrls array

    if (proxyUrls && !proxyUrls.length) throw new Error('Parameter "options.proxyUrls" of type Array must not be empty');
    (0, _utils.checkParamOrThrow)(useLiveView, 'options.useLiveView', 'Maybe Boolean');
    (0, _utils.checkParamOrThrow)(sessionPool, 'options.sessionPool', 'Maybe Object'); // Config.
    ...
    this.recycledDiskCacheDirs = recycleDiskCache ? new _linked_list.default() : null;
    this.useIncognitoPages = useIncognitoPages;
    this.proxyUrls = proxyUrls ? _underscore.default.shuffle(proxyUrls) : null;
    this.liveViewServer = useLiveView ? new _live_view_server.default() : null;

    this.launchPuppeteerFunction = async () => {
      // Do not modify passed launchPuppeteerOptions!
      const opts = _underscore.default.clone(launchPuppeteerOptions) || {};
      opts.args = _underscore.default.clone(opts.args || []); // If requested, use recycled disk cache directory

      let diskCacheDir = null;

      if (recycleDiskCache) {
        diskCacheDir = this.recycledDiskCacheDirs.length > 0 ? this.recycledDiskCacheDirs.removeFirst() : await mkdtempAsync(DISK_CACHE_DIR);
    ...
    this.instanceKillerInterval = setInterval(() => this._killRetiredInstances(), instanceKillerIntervalMillis || instanceKillerIntervalSecs * 1000);
    this.idlePages = []; // WeakSet/Map items do not prevent garbage collection,
    ...
    this.liveViewSnapshotsInProgress = new WeakMap(); // ensure termination on SIGINT
    ...
    if (sessionPool) {
      ...
      sessionPool.on(_events.default.SESSION_RETIRED, this._retireBrowserWithSession);
    }
  }
  /**
  ...
  _launchInstance() {
    ...
    _log.default.debug('PuppeteerPool: Launching new browser', {
      id
    });

    const browserPromise = this.launchPuppeteerFunction();
    const instance = new PuppeteerInstance(id, browserPromise);
    this.activeInstances[id] = instance; // Handle the async stuff elsewhere.
  ...
  async _initBrowser(browserPromise, instance) {
    ...
    try {
      ...
      _log.default.exception(err, 'PuppeteerPool: Browser launch failed', {
        id
      });
    ...
    browser.on('disconnected', () => {
      ...
      if (!instance.killed) _log.default.error('PuppeteerPool: Puppeteer sent "disconnect" event. Maybe it crashed???', {
        id
      });
  ...
  _retireInstance(instance) {
    ...
    if (!this.activeInstances[id]) return _log.default.debug('PuppeteerPool: browser is retired already', {
      id
    });

    _log.default.debug('PuppeteerPool: Retiring browser', {
      id
    });
  ...
  _killRetiredInstances() {
    _log.default.debug('PuppeteerPool: Retired browsers count', {
      count: _underscore.default.values(this.retiredInstances).length
    });

    _underscore.default.mapObject(this.retiredInstances, async instance => {
      ...
      if (Date.now() - instance.lastPageOpenedAt > this.killInstanceAfterMillis) {
        _log.default.debug('PuppeteerPool: killing retired browser after period of inactivity', {
          id: instance.id,
          killInstanceAfterMillis: this.killInstanceAfterMillis
      ...
      try {
        ...
        const pages = await browser.pages(); // NOTE: we are killing instance when the number of pages is less or equal to 1 because there is always about:blank page.

        if (pages.length <= 1) {
          _log.default.debug('PuppeteerPool: Killing retired browser because it has no open tabs', {
            id: instance.id
          });
        ...
        _log.default.exception(err, 'PuppeteerPool: Browser.pages() failed', {
          id: instance.id
        });
  ...
  async _killInstance(instance) {
    ...
    _log.default.debug('PuppeteerPool: Killing browser', {
      id
    });
    ...
    const recycleDiskCache = () => {
      ...
      _log.default.debug('PuppeteerPool: Recycling disk cache dir', {
        id,
        diskCacheDir: instance.recycleDiskCacheDir
      });
    ...
    try {
      ...
      _log.default.exception(err, 'PuppeteerPool: Cannot close the browser instance, it will be killed forcibly.', {
        id
      });
    }
  }
  ...
  async newPage() {
    ...
    let idlePage; // We don't need to check whether options.reusePages is true,
  ...
  async _openNewTab() {
    let instance = Object.values(this.activeInstances).find(inst => inst.activePages < this.maxOpenPagesPerInstance);
    ...
    try {
      const browser = await instance.browserPromise;
      const context = this.useIncognitoPages ? await browser.createIncognitoBrowserContext() : browser.defaultBrowserContext();
      const page = await (0, _utils2.addTimeoutToPromise)(context.newPage(), this.puppeteerOperationTimeoutMillis, 'PuppeteerPool: browser.newPage() timed out.');
      await this._focusOldestTab(browser).catch(() => _log.default.debug('Could not focus oldest tab.'));
  ...
  _decoratePage(page) {
    ...
    page.close = async (...args) => {
      ...
      await originalPageClose.apply(page, args).catch(err => {
        _log.default.debug('PuppeteerPool: Page.close() failed', {
          errorMessage: err.message,
          id: instance.id
        });
      });
      ...
      if (context.isIncognito()) {
        await context.close().catch(err => {
          _log.default.debug('PuppeteerPool: Context.close() failed', {
            errorMessage: err.message,
            id: instance.id
          });
        });
    ...
    page.once('error', error => {
      _log.default.exception(error, 'PuppeteerPool: Page crashed.');
  ...
  async destroy() {
    ...
    process.removeListener('SIGINT', this.sigintListener); // TODO: delete of dir doesn't seem to work!

    const browserPromises = _underscore.default.values(this.activeInstances).concat(_underscore.default.values(this.retiredInstances)).map(instance => {
    ...
    try {
      await Promise.all(closePromises); // Delete all cache directories
      ...
      _log.default.exception(err, 'PuppeteerPool: Cannot close the browsers.');
    }

    if (this.liveViewServer) {
      await this.liveViewServer.stop().catch(err => _log.default.exception(err, 'PuppeteerPool: Cannot close LiveViewServer.'));
    }
  }
  /**
  ...
  async retire(browser) {
    ...
    _log.default.debug('PuppeteerPool: Browser is retired already');
  }
  /**
  ...
  async recyclePage(page) {
    ...
    if (this.reusePages) {
      ...
      try {
        await (0, _utils2.addTimeoutToPromise)(page.close(), this.puppeteerOperationTimeoutMillis, 'PuppeteerPool: page.close() timed out.');
      } catch (err) {
        _log.default.debug('PuppeteerPool: page.close() failed.', {
  ...
  async serveLiveViewSnapshot(page) {
    ...
    const pages = await browser.pages(); // We only serve the second page of the browser because it's in focus,
    // which is necessary for screenshots. First page is about:blank.

    if (pages[1] !== page) return;
    const instance = await this._findInstanceByBrowser(browser); // Sometimes the browser gets killed and there's no instance.

    if (!instance) return; // Only take snapshots in the most recently opened browser.

    if (instance.id !== this.browserCounter - 1) return;
    const snapshotPromise = this.liveViewServer.serve(page).catch(err => _log.default.debug('Live View failed to be served.', {
      message: err.message
    }));
  ...
  async _retireBrowserWithSession(session) {
    ...
    if (isInstanceRunning) {
      ...
      try {
        ...
        _log.default.debug('Could not retire instances ', e);
      }
    }
  }
  ...
  _killInstanceWithNoPages(instance) {
    ...
    if (instance.activePages === 0 && this.retiredInstances[id]) {
      ...
      setTimeout(() => {
        _log.default.debug('PuppeteerPool: Killing retired browser because it has no active pages', {
          id
        });
